...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .do-head-icon--tiktok { background:var(--tiktok-light); }
  .do-head-icon--tiktok svg { stroke:#EE1D52; }
  ...
  .ms-tab-btn.active { background:linear-gradient(135deg,var(--tiktok) 0%,var(--tiktok-dark) 100%); color:#fff; box-shadow:0 4px 12px rgba(238,29,82,.25); }
  ...
  /* POST CARD LIST (sama seperti most-engagement) */
  ...
  /* THUMBNAIL */
  .fme-post-thumb { width:100px; height:160px; border-radius:10px; flex-shrink:0; overflow:hidden; border:2px solid var(--border-gray); background:#1a202c; position:relative; align-self:center; box-shadow:0 4px 12px rgba(0,0,0,.08); }
  ...
  .fme-post-thumb-placeholder { width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:28px; background:linear-gradient(135deg,#1a202c,#374151); }
  ...
  .fme-pag-btn:hover:not(:disabled):not(.is-active) { border-color:var(--tiktok); color:var(--tiktok); background:var(--tiktok-light); }
  .fme-pag-btn.is-active { background:var(--tiktok); border-color:var(--tiktok); color:#fff; }
  ...
  .modal-header { padding:18px 24px; border-bottom:1px solid var(--border-gray); display:flex; justify-content:space-between; align-items:center; flex-shrink:0; background:linear-gradient(135deg,var(--tiktok) 0%,var(--tiktok-dark) 100%); }
  ...
  .modal-profile { display:flex; align-items:center; gap:14px; padding-bottom:18px; border-bottom:1px solid var(--bg-gray-100); margin-bottom:18px; }
  .modal-avatar { width:56px; height:56px; border-radius:50%; border:2.5px solid var(--border-gray); background:linear-gradient(135deg,var(--tiktok),var(--tiktok-dark)); display:flex; align-items:center; justify-content:center; color:#fff; font-weight:700; font-size:20px; flex-shrink:0; overflow:hidden; }
  .modal-avatar img { width:100%; height:100%; border-radius:50%; object-fit:cover; }
  .modal-author-name   { font-size:17px; font-weight:700; color:var(--text-primary); margin:0 0 3px; }
  .modal-author-handle { font-size:12px; color:var(--text-secondary); font-weight:500; margin-bottom:8px; }
  .modal-badges { display:flex; gap:8px; flex-wrap:wrap; }
  .modal-post-content { font-size:14px; line-height:1.7; color:var(--text-primary); margin-bottom:18px; word-break:break-word; white-space:pre-wrap; }
  .modal-post-meta { display:flex; align-items:center; gap:12px; padding:14px 0; border-top:1px solid var(--bg-gray-100); border-bottom:1px solid var(--bg-gray-100); margin-bottom:18px; font-size:12px; color:var(--text-secondary); }
  .modal-post-meta svg { width:13px; height:13px; stroke:currentColor; fill:none; }
  ...
  .modal-stat-value.v-views { color:var(--tiktok); }
  ...
  .modal-emo-title { font-size:11px; font-weight:700; color:var(--text-secondary); text-transform:uppercase; letter-spacing:.5px; margin-bottom:10px; }
  ...
  .modal-btn.primary   { background:linear-gradient(135deg,var(--tiktok),var(--tiktok-dark)); color:#fff; box-shadow:0 4px 14px rgba(238,29,82,.25); }
  .modal-btn.primary:hover { transform:translateY(-2px); box-shadow:0 6px 20px rgba(238,29,82,.35); }
  ...
  /* Section heading style used in modal */
...
<div class="fme-page">
  ...
  <div class="page-header">
    <div class="page-header-left">
      <h1>
        <svg viewBox="0 0 24 24" style="width:26px;height:26px;display:inline-block;vertical-align:middle;margin-right:10px;margin-top:-3px;fill:none;stroke:#EE1D52;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;">
          <circle cx="12" cy="12" r="10"/><path d="M12 2a10 10 0 0 1 10 10"/><path d="M8 12l2 2 4-4"/>
        </svg>
        TikTok Emotion Analysis
      </h1>
      <p>Analisis pola emosi pada postingan TikTok berdasarkan Plutchik Wheel — joy, trust, fear, surprise, sadness, disgust, anger, anticipation</p>
    </div>
  ...
  <div class="filter-card">
    <form id="filterForm" method="GET" action="{{ route('mk.tiktok.emotion-analysis') }}">
  ...
  <div class="ms-grid-2 ms-mb20">
    <div class="do-card do-card--tiktok">
      <div class="do-card-head">
        ...
        <span class="do-badge do-badge--tiktok" id="barBadge">—</span>
      </div>
  ...
  <div class="do-card do-card--tiktok ms-mb20">
    <div class="do-card-head">
      ...
      <span class="do-badge do-badge--tiktok">Trends</span>
    </div>
    <div class="do-card-body">
      <div class="ms-ch-340">
      ...
      <div style="display:flex;gap:14px;padding:16px 20px;border-bottom:1px solid #f1f5f9;align-items:flex-start;">
        ...
        <div style="flex:1;">
          ...
          <div style="display:flex;gap:6px;">
            ...
            <div class="loading-skeleton" style="height:22px;width:70px;border-radius:20px;"></div>
            <div class="loading-skeleton" style="height:22px;width:70px;border-radius:20px;"></div>
            <div class="loading-skeleton" style="height:22px;width:80px;border-radius:20px;"></div>
          </div>
        </div>
        <div class="loading-skeleton" style="width:100px;height:160px;border-radius:10px;flex-shrink:0;"></div>
      </div>
      @endfor
    </div>
...
/* FIELD HELPERS — sama persis dengan most-engagement */
...
function getAvatar(item) {
  return (item.avatar_url||item.profile_url||item.author?.image||item.author_avatar||item.profile_image||'').trim();
}
function getThumbnail(item) {
  return (item.avatar_url||item.profile_url||item.author_avatar||item.profile_image||item.thumbnail_url||item.cover_url||item.thumbnail||'').trim();
}
...
function getAvatarColor(item) {
  ...
  const palette=['#EE1D52','#c4163f','#10b981','#f59e0b','#8b5cf6','#06b6d4','#ec4899','#14b8a6','#f97316','#6366f1'];
  let h=0; for(let i=0;i<seed.length;i++) h=(h*31+seed.charCodeAt(i))&0xffffffff;
  return palette[Math.abs(h)%palette.length];
}
/* ✅ field metrics sama persis dengan most-engagement controller */
...
const EmoApp = {
  ...
  async loadData() {
    ...
    const url=`/mk/api/tiktok/most-engagement?project_id=${EmoCfg.pid}&start_date=${EmoCfg.sd}&end_date=${EmoCfg.ed}&sub=postbyview&rows=200`;
  ...
  /* ── RENDER LIST (card style, sama seperti most-engagement) ── */
  renderList() {
  ...
  /* ── POST CARD HTML (mirip most-engagement + emotion badge) ── */
  postHTML(post, rank) {
    ...
    const content = decodeStr((post.content||'').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()).slice(0,200);
    const dt      = (post.date_created||'').split('T')[0];
    const url     = post.url||post.link||'';

    /* ✅ Gunakan helper functions — field sama dengan most-engagement */
    ...
    return `<div class="fme-post">
      ...
      <div class="fme-post-body">
        ...
        <div class="fme-post-stats">
          ...
          <span class="fme-post-metric fme-post-metric--tiktok"><svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>${numFmt(views)}</span>
          ...
          ${url?`<a href="${esc(url)}" target="_blank" rel="noopener" class="fme-view-link" onclick="event.stopPropagation()"><svg viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>TikTok</a>`:''}
        </div>
      </div>
      ${thumbHtml}
    </div>`;
  ...
  /* MODAL — full detail popup sama seperti most-engagement */
  openModal(post) {
    if(!post) return;
    const name    = getName(post);
    const av      = getAvatar(post);
    ...
    const safeIni = ini.replace(/['"\\]/g,'');
    const handle  = post.author?.scr_name||post.author_scr_name||'';
    const content = decodeStr((post.content||'').replace(/<[^>]*>/g,'').trim());
    const url     = post.url||post.link||'';

    /* ✅ field sama dengan most-engagement */
    const views = getViews(post);
    const likes = getLikes(post);
    const cmts  = getComments(post);
    ...
    if(!videoId&&post.id){ const m=String(post.id).match(/(\d{10,})/); if(m) videoId=m[1]; }
      ...
      <div>
        <div class="msdp-section-heading">
          <div class="msdp-section-heading-icon" style="background:rgba(238,29,82,.08);">
            <svg viewBox="0 0 24 24" style="stroke:#EE1D52;stroke-width:2;fill:none;stroke-linecap:round;"><polygon points="5 3 19 12 5 21 5 3"/></svg>
          </div>
        ...
        <div style="border-radius:14px;overflow:hidden;border:1px solid #e8edf2;box-shadow:0 4px 16px rgba(0,0,0,.07);">
          <iframe id="emoVideoFrame" src="https://www.tiktok.com/embed/v2/${videoId}" allow="autoplay" allowfullscreen style="width:100%;height:560px;border:none;display:block;"></iframe>
    ...
    document.getElementById('modalBody').innerHTML=`
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div style="display:flex;align-items:center;gap:14px;padding:16px 18px;background:linear-gradient(135deg,#fafbfc,#f1f5f9);border:1px solid #e8edf2;border-radius:14px;">
          <div class="modal-avatar" style="width:56px;height:56px;font-size:20px;font-weight:800;background:linear-gradient(135deg,${color},${color}99);border:3px solid rgba(255,255,255,.9);box-shadow:0 4px 16px rgba(0,0,0,.18);">${avHtml}</div>
        ...
        <div>
          <div class="msdp-section-heading">
            <div class="msdp-section-heading-icon" style="background:rgba(238,29,82,.08);">
              <svg viewBox="0 0 24 24" style="stroke:#EE1D52;stroke-width:2;fill:none;stroke-linecap:round;"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
            </div>
        ...
        <div class="modal-actions">
          ${url?`<a href="${esc(url)}" target="_blank" rel="noopener" class="modal-btn primary"><svg viewBox="0 0 24 24" style="width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>Buka di TikTok</a>`:''}
  ...
  exportCSV() {
    ...
    const header='index;creator;handle;emosi;sentimen;views;likes;comments;tanggal;url;konten';
    ...
    a.download=`TikTok_EmotionAnalysis_${EmoCfg.sd}_${EmoCfg.ed}.csv`; a.click();
  },
  ...
  renderRadarChart() {
    ...
    chart.setOption({
      ...
      series:[{ type:'radar', data:[{ value:EMOTIONS.map(e=>counts[e]||0), name:'Emotion',
        areaStyle:{color:{type:'linear',x:0,y:0,x2:1,y2:1,colorStops:[{offset:0,color:'rgba(238,29,82,0.18)'},{offset:1,color:'rgba(196,22,63,0.08)'}]}},
        lineStyle:{color:'#EE1D52',width:2.5}, symbol:'circle', symbolSize:6, itemStyle:{color:EMOTIONS.map(e=>EMO_COLORS[e]),borderColor:'#fff',borderWidth:2} }] }]
    });
  },
