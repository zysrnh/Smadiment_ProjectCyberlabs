...
<style>
  ...
  @media (max-width: 640px) {
    ...
    .date-picker-container { width: 100%; max-height: 100vh; border-radius: 0; }
    .date-picker-sidebar { border-radius: 0; }
...
<div class="dashboard-container">
  ...
  <div class="page-header">
    <h1>TikTok Overview Dashboard</h1>
    <p>Monitor and analyze your TikTok social media performance metrics</p>
  </div>
  ...
  <div class="alert alert-warning">
    ...
    <span>No project selected. Please select a project from the sidebar to view TikTok Overview data.</span>
  </div>
  @else

  <!-- Date Filter Card -->
  <div class="filter-card">
    <form id="filterForm" method="GET" action="{{ route('mk.tiktok.overview') }}">
...
if (projectId && startDate && endDate) {
  ...
  async function loadVolumeTotal() {
    ...
    try {
      const response = await fetch(`/mk/api/tiktok/volume-total?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
      ...
      cards.forEach(card => { removeLoadingBadge(card); card.classList.add('loaded'); });
    }
  }
  ...
  async function loadSentimentTotal() {
    ...
    try {
      const response = await fetch(`/mk/api/tiktok/sentiment-total?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
      const result = await response.json();
  ...
  async function loadMostActiveUsers() {
    ...
    try {
      const response = await fetch(`/mk/api/tiktok/most-active-users?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
      const result = await response.json();
  ...
  function buildUsersTableHTML(users) {
    ...
    users.forEach((item, index) => {
      ...
      const displayName = item.contentJson?.nickname || item.display_name || item.name || username;
      const followers   = item.followers || item.contentJson?.follower_count || 0;
      const likes       = item.likes    || item.contentJson?.heart_count || 0;
      const comments    = item.comments || 0;
      const shares      = item.shares   || 0;
      const totalPosts  = item.posts    || item.y || 0;
      const engagement  = totalPosts + likes + comments + shares;
        ...
        <tr>
          ...
          <td>
            <div class="avatar-container">
              ${profileUrl
                ? `<img src="${profileUrl}" alt="${username}" class="user-avatar-img"
          ...
          <td>
            <a href="https://www.tiktok.com/@${username}" target="_blank" class="username-link">@${username}</a>
          </td>
          <td>
            <span class="account-name-link">${displayName}</span>
          </td>
  ...
  function displayUsersTable(count) {
    const container = document.getElementById('activeUsersTable');
    container.innerHTML = buildUsersTableHTML(allUsers.slice(0, count));
  ...
  function filterUsers() {
    ...
    const filteredUsers = allUsers.filter(user => {
      ...
      const displayName = (user.contentJson?.nickname || user.display_name || user.name || '').toLowerCase();
    ...
    if (filteredUsers.length === 0) {
      ...
      container.innerHTML = buildUsersTableHTML(filteredUsers);
    }
  }
  ...
  function animateProgress(card, percentage) {
    const bar = card ? card.querySelector('.stat-progress-bar') : null;
    ...
    new Chart(canvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor: '#1a202c', padding: 16, titleColor: '#fff', bodyColor: '#fff',
      ...
      options: {
        ...
        plugins: {
          legend: {
            position: 'bottom',
            labels: {
              color: '#1a202c', font: { family: 'Poppins', size: 13, weight: '600' },
