...
<div class="page-export-bar" data-html2canvas-ignore="true">
    ...
    <div class="page-export-bar-right">
        ...
        <button type="button" class="page-export-btn page-export-btn-img"
                id="pageExportImgBtn"
                onclick="WCExport.run('image', this)"
...
function renderWC() {
    ...
    /* Tunggu satu frame agar container benar-benar rendered di DOM
       sebelum echarts mengukur dimensinya */
    requestAnimationFrame(() => {
        /* Sembunyikan loading setelah frame */
        if (ld) ld.style.display = 'none';

        const rect = ch.getBoundingClientRect();
        /* Jika container belum punya ukuran, coba lagi setelah 100ms */
        ...
        /* ResizeObserver → chart ikut saat container berubah ukuran */
...
function renderList() {
    ...
    items.forEach((h, i) => {
        ...
        el.onclick = () => { const q = encodeURIComponent(h.name.replace(/^#/,'')); window.open(`https://www.tiktok.com/tag/${q}`,'_blank','noopener,noreferrer'); };
        list.appendChild(el);
    });
    if (pg) {
...
const WCExport = (() => {
    ...
    async function runCard(areaId, cardKey, type, btn) {
        ...
        try {
            ...
            const titles  = { wordcloud: 'Word Cloud', topics: 'Top Topics' };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    /* Global resize fallback untuk browser tanpa ResizeObserver */
