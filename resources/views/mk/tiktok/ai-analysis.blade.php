...
<style>
    :root {
        ...
        --tt-btn:      linear-gradient(135deg, #EE1D52 0%, #69C9D0 100%);
        --sidebar-w:   260px;
    }
    ...
    .prompt-item:hover { background: rgba(238,29,82,0.06); color: var(--primary); }
    .prompt-item.active {
        background: rgba(238,29,82,0.1); color: var(--primary);
    ...
    .ai-avatar {
        ...
        background: var(--tt-black); display: flex; align-items: center;
        justify-content: center; flex-shrink: 0; position: relative; overflow: hidden;
    }
    .ai-avatar::before {
        content: ''; position: absolute; inset: 0;
        background: var(--tt-gradient); opacity: 0.2;
    }
    ...
    .date-picker-trigger {
        ...
        border-radius: 9px; border: 1px solid var(--border); background: #f8fafc;
        cursor: pointer; font-size: 11.5px; font-weight: 600; color: var(--text-primary);
    ...
    .active-prompt-bar {
        padding: 7px 18px; background: rgba(238,29,82,0.06);
        border-bottom: 1px solid rgba(238,29,82,0.12);
    ...
    .welcome-icon-wrap {
        width: 56px; height: 56px; border-radius: 16px; background: var(--tt-black);
        display: flex; align-items: center; justify-content: center;
        box-shadow: -4px 4px 0 var(--tt-cyan), 4px -4px 0 var(--primary);
    }
    ...
    .btn-send {
        ...
        background: var(--tt-btn); color: #fff; cursor: pointer;
        display: flex; align-items: center; justify-content: center;
        box-shadow: 0 3px 10px rgba(238,29,82,0.32); transition: all 0.15s; flex-shrink: 0;
    }
    ...
    <div class="ai-main">
        ...
        <div class="ai-messages" id="aiMessages">
            <div class="welcome-state" id="welcomeState">
                ...
                <p>Pilih template dari panel kiri, atau ketik pertanyaan sendiri untuk menganalisis data percakapan TikTok pada project ini.</p>
...
// ═══════════════════════════════════════════════════════════════════
// SIDEBAR PROMPT GROUPS — identik dengan Facebook
// ═══════════════════════════════════════════════════════════════════
const PROMPT_GROUPS = [
    { key: 'isu',       label: 'Analisis Isu',         color: '#EE1D52', keys: ['butterfly_effect','isu_positif_negatif','isu_swot','analisis_percakapan','analisis_agenda_setting','narrative_analysis'] },
    ...
    { key: 'strategi',  label: 'Strategi & Komunikasi', color: '#69C9D0', keys: ['pestle','uses_gratifications','stakeholder_mapping','strategi_riding_the_wave','strategi_counter_narrative','analisis_isu_parpol'] },
...
// ═══════════════════════════════════════════════════════════════════
// PRELOAD DATA — sama persis pola Facebook
// ═══════════════════════════════════════════════════════════════════
async function preloadProjectData() {
    ...
    try {
        ...
        setReady(
            `🎵 ${s.total_posts} videos · +${Math.round(pos/tot*100)}% / -${Math.round(neg/tot*100)}% · ` +
            `${s.total_hashtags} hashtags · ${START_DATE} → ${END_DATE}`
        );
    } catch (err) {
        cachedDataset = `Project ID: ${PROJECT_ID}, Platform: TikTok, Periode: ${START_DATE} s/d ${END_DATE}`;
        dataReady = true;
...
// ═══════════════════════════════════════════════════════════════════
// DATE PICKER — identik dengan Facebook
// ═══════════════════════════════════════════════════════════════════
...
(() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('datePickerTrigger')?.addEventListener('click', () => {
            document.getElementById('datePickerModal').classList.add('show'); both();
        });
        ...
        document.querySelectorAll('.date-preset').forEach(btn => {
            btn.addEventListener('click', () => {
                document.querySelectorAll('.date-preset').forEach(b => b.classList.remove('active'));
                btn.classList.add('active'); preset(btn.dataset.preset);
            });
        });
...
async function sendMessage() {
    ...
    const typingEl = appendTyping('Menganalisis data TikTok…');
...
// ═══════════════════════════════════════════════════════════════════
// UI HELPERS — identik dengan Facebook
// ═══════════════════════════════════════════════════════════════════
function appendMsg(role, text) {
    ...
    const ava = `width:32px;height:32px;min-width:32px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0;color:#fff !important;background:${isAI?'linear-gradient(135deg,#038047 0%,#026738 100%)':'linear-gradient(135deg,#EE1D52 0%,#69C9D0 100%)'} !important;box-shadow:${isAI?'0 2px 8px rgba(3,128,71,.25)':'0 2px 8px rgba(238,29,82,.25)'};`;
    const bub = isAI
