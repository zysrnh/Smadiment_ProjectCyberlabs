...
{{-- KPI — 4 Rotating Cards (8 emotions, 2 sets × 4) --}}
<style>
...
.kpi-rot-dot {
    ...
    background:var(--slate-300); transition:background .3s, width .3s;
    cursor:pointer; border:none; padding:0;
}
.kpi-rot-dot.active { background:var(--primary); width:18px; border-radius:3px; }
.kpi-rot-progress {
    position:absolute; bottom:0; left:0; height:2px;
    background:rgba(255,255,255,.55); border-radius:0 0 var(--radius) 0;
    transition:none; animation:kpiRotProg 3s linear forwards;
}
...
<div class="kpi-rot-wrap mb-3" id="kpiRotWrap">
    ...
    <div class="kpi-rot-set visible" id="kpiSet0">
        ...
        @endphp
        @foreach($rotCards as $rc)
            @if($rc[0] === 0)
            <div class="card h-100 text-white kpi-card-hover" style="background:{{ $rc[3] }};animation:fadeUp .38s ease-out {{ $loop->index * 0.05 }}s both;position:relative;overflow:hidden;">
                <div class="kpi-rot-progress" id="kpiProg{{ $loop->index }}"></div>
                <div class="card-body">
    ...
    {{-- Set 2: Sadness, Disgust, Anger, Anticipation --}}
...
<div class="kpi-rot-dots mb-3">
    <button class="kpi-rot-dot active" onclick="KPIRot.goTo(0)" id="kpiDot0" title="Joy · Trust · Fear · Surprise"></button>
    <button class="kpi-rot-dot" onclick="KPIRot.goTo(1)" id="kpiDot1" title="Sadness · Disgust · Anger · Anticipation"></button>
</div>

<script>
const KPIRot = (() => {
    const SETS = 2, INTERVAL = 3000;
    let cur = 0, timer = null;

    function goTo(idx) {
        const sets = document.querySelectorAll('.kpi-rot-set');
        sets.forEach((s, i) => {
            s.classList.toggle('visible', i === idx);
            s.classList.toggle('hidden',  i !== idx);
        });
        document.querySelectorAll('.kpi-rot-dot').forEach((d, i) => d.classList.toggle('active', i === idx));
        cur = idx;
        /* restart progress bars in active set */
        sets[idx].querySelectorAll('.kpi-rot-progress').forEach(p => {
            p.style.animation = 'none';
            p.offsetHeight; /* reflow */
            p.style.animation = `kpiRotProg ${INTERVAL}ms linear forwards`;
        });
    }

    function next() { goTo((cur + 1) % SETS); }
    ...
    // pause on hover
    const wrap = document.getElementById('kpiRotWrap');
    if (wrap) {
...
<div class="page-export-bar" data-html2canvas-ignore="true">
    <div class="page-export-bar-left">
        ...
        <span class="badge bg-light-secondary text-muted ms-1" style="font-size:10px;">KPI + Charts + Tweet List</span>
    </div>
