...
.ht-bar-wrap{flex:0 0 60px;height:6px;background:var(--slate-100);border-radius:99px;overflow:hidden}.ht-bar-fill{height:100%;border-radius:99px;background:linear-gradient(90deg,#D97706,rgba(217,119,6,.5));transition:width .4s cubic-bezier(.4,0,.2,1)}.ht-count{font-size:11px;font-weight:700;color:var(--slate-500);white-space:nowrap;flex-shrink:0;min-width:36px;text-align:right}
...
const DONUT_COLORS=['#D97706','#EA580C','#DC2626','#7C3AED','#2563EB'];
...
function showEmpty(){_$('loadingDonut').style.display='none';_$('donutEmpty').style.display='flex';_$('donutChart').style.display='none';_$('userList').innerHTML='<div class="spinner-state" style="padding:40px;"><i class="ph ph-users" style="font-size:32px;color:var(--slate-300);"></i><span>Tidak ada data influencer</span></div>';_$('pagArea').innerHTML='';['kpiTotal','kpiEng','kpiAvgFol'].forEach(i=>{const e=_$(i);if(e)e.textContent='0'});_$('kpiTopAcc').textContent='–';_$('badgeTotal').textContent='0'}
function updateKpi(){
