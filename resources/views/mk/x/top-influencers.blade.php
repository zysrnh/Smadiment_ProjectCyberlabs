...
/* ═══════════════════════════════════════════════════════════
   EXPORT MODULE — X Top Influencers
   Supports: full page (KPI + Donut + List), per-card
═══════════════════════════════════════════════════════════ */
const XIExport = (() => {
    ...
    /* ── PDF header helper ── */
    function _drawPdfHeader(doc, pW, margin, title) {
    ...
    /* ── Paginate canvas into PDF ── */
    function _paginateCanvas(canvas, pdf, label) {
        ...
        while(srcY < canvas.height){
            if(pageNum>0){ pdf.addPage(); _drawPdfHeader(pdf,pW,margin,label); }
    ...
    async function _exportPdf() {
        ...
        const pW  = pdf.internal.pageSize.getWidth();
        _drawPdfHeader(pdf, pW, 10, 'X Top Influencers');
        _paginateCanvas(canvas, pdf, 'X Top Influencers');
    ...
    async function _exportImage() {
        ...
        link.download = `x_influencers_${CFG.pid}_${stamp}.png`;
    ...
    /* ── Per-card label/filename ── */
    const _cardMeta = {
        'donut' : { label:'Top 5 Engagement Share', file:'top5-engagement-share' },
        'list'  : { label:'Influencer Ranking',     file:'influencer-ranking'    },
    };
    function _cardLabel(k)    { return (_cardMeta[k]||{label:k}).label; }
    function _cardFilename(k) {
        const f=(_cardMeta[k]||{file:k}).file;
        const stamp=new Date().toISOString().slice(0,10).replace(/-/g,'');
        return `x_influencers_${f}_${CFG.pid}_${stamp}`;
    }
    ...
    async function runCard(areaId, cardKey, type, btn) {
        ...
        try {
            ...
            if(type==='image'){
                ...
                const pW  = pdf.internal.pageSize.getWidth();
                _drawPdfHeader(pdf, pW, 10, _cardLabel(cardKey));
                _paginateCanvas(canvas, pdf, _cardLabel(cardKey));
                pdf.save(fname+'.pdf');
