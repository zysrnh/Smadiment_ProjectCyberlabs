...
.kpi-card-hover h3{font-size:1.5rem}@media(max-width:640px){.do-panel{width:100vw}.tme-tabs{flex-wrap:wrap}.tme-tab-btn{flex:unset;min-width:calc(50% - 4px)}.tme-post-thumb{display:none}}
</style>
@endsection
...
{{-- KPI --}}
<div class="row mb-3">
    <div class="col-md-6 col-xl-3"><div class="card h-100 bg-primary text-white kpi-card-hover fade-up fade-up-d1"><div class="card-body"><div class="d-flex align-items-center"><div class="flex-grow-1"><p class="mb-1 text-white text-opacity-75 f-12">Total Views</p><h3 class="mb-0 text-white f-w-300" id="kpiViews"><span class="sk-block" style="width:80px;height:24px;display:inline-block;"></span></h3><p class="mb-0 mt-2 text-white text-opacity-75 f-12" id="kpiViewsSub"><i class="ph ph-eye me-1"></i>Loading…</p></div><div class="flex-shrink-0 ms-3"><div class="kpi-icon-bg"><i class="ph ph-eye"></i></div></div></div></div></div></div>
    <div class="col-md-6 col-xl-3"><div class="card h-100 bg-success text-white kpi-card-hover fade-up fade-up-d2"><div class="card-body"><div class="d-flex align-items-center"><div class="flex-grow-1"><p class="mb-1 text-white text-opacity-75 f-12">Total Retweets</p><h3 class="mb-0 text-white f-w-300" id="kpiRt"><span class="sk-block" style="width:80px;height:24px;display:inline-block;"></span></h3><p class="mb-0 mt-2 text-white text-opacity-75 f-12" id="kpiRtSub"><i class="ph ph-repeat me-1"></i>Loading…</p></div><div class="flex-shrink-0 ms-3"><div class="kpi-icon-bg"><i class="ph ph-repeat"></i></div></div></div></div></div></div>
    <div class="col-md-6 col-xl-3" style="display:none;"><div class="card h-100 bg-warning text-white kpi-card-hover fade-up fade-up-d3"><div class="card-body"><div class="d-flex align-items-center"><div class="flex-grow-1"><p class="mb-1 text-white text-opacity-75 f-12">Total Likes</p><h3 class="mb-0 text-white f-w-300" id="kpiLikes"><span class="sk-block" style="width:80px;height:24px;display:inline-block;"></span></h3><p class="mb-0 mt-2 text-white text-opacity-75 f-12" id="kpiLikesSub"><i class="ph ph-heart me-1"></i>Loading…</p></div><div class="flex-shrink-0 ms-3"><div class="kpi-icon-bg"><i class="ph ph-heart"></i></div></div></div></div></div></div>
    <div class="col-md-6 col-xl-3" style="display:none;"><div class="card h-100 bg-danger text-white kpi-card-hover fade-up fade-up-d4"><div class="card-body"><div class="d-flex align-items-center"><div class="flex-grow-1"><p class="mb-1 text-white text-opacity-75 f-12">Total Replies</p><h3 class="mb-0 text-white f-w-300" id="kpiReplies"><span class="sk-block" style="width:80px;height:24px;display:inline-block;"></span></h3><p class="mb-0 mt-2 text-white text-opacity-75 f-12" id="kpiRepliesSub"><i class="ph ph-chat-circle me-1"></i>Loading…</p></div><div class="flex-shrink-0 ms-3"><div class="kpi-icon-bg"><i class="ph ph-chat-circle"></i></div></div></div></div></div></div>
</div>
...
<div class="tme-tabs" id="ovTabsBar">
    <button class="tme-tab-btn active" id="tab-hashtag" onclick="OVTab.show('hashtag')"><i class="ph ph-hash"></i> Top Topics <span class="tme-tab-chip" id="chip-hashtag">—</span></button>
    ...
    <button class="tme-tab-btn" id="tab-retweet" onclick="OVTab.show('retweet')"><i class="ph ph-repeat"></i> Most Retweeted <span class="tme-tab-chip" id="chip-retweet">—</span></button>
    <button class="tme-tab-btn" id="tab-like" onclick="OVTab.show('like')"><i class="ph ph-heart"></i> Most Liked <span class="tme-tab-chip" id="chip-like">—</span></button>
    <button class="tme-tab-btn" id="tab-reply" onclick="OVTab.show('reply')"><i class="ph ph-chat-circle"></i> Most Replies <span class="tme-tab-chip" id="chip-reply">—</span></button>
</div>
            ...
            <div class="card mb-3" style="animation:fadeUp .38s ease-out .22s both;">
                ...
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div><h6 class="mb-0">Distribusi — Top 5</h6><small class="text-muted">Proporsi penggunaan topic teratas</small></div>
                    </div>
...
{{-- Engagement Panels --}}
@foreach(['view','retweet','like','reply'] as $tp)
@php $panelLabels=['view'=>'Most Viewed','retweet'=>'Most Retweeted','like'=>'Most Liked','reply'=>'Most Replies'];$panelIcons=['view'=>'ph-eye','retweet'=>'ph-repeat','like'=>'ph-heart','reply'=>'ph-chat-circle']; @endphp
<div class="tme-tab-panel" id="panel-{{ $tp }}">
    ...
    <div class="card mb-3" style="animation:fadeUp .38s ease-out .18s both;">
        ...
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div class="d-flex align-items-center gap-2">
                <div class="avtar avtar-xs bg-light-primary rounded"><i class="ph ph-chart-donut f-18 text-primary"></i></div>
                <div><h6 class="mb-0">Distribusi {{ $panelLabels[$tp] }} — Top 5</h6><small class="text-muted">Proporsi engagement per tweet — klik untuk detail</small></div>
            </div>
            <div class="d-flex align-items-center gap-2 flex-wrap">
    ...
    <div class="card mb-3" style="animation:fadeUp .38s ease-out .22s both;">
        ...
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div class="d-flex align-items-center gap-2">
                ...
                <div><h6 class="mb-0">Top Tweets by {{ $panelLabels[$tp] }}</h6><small class="text-muted">Klik tweet untuk lihat detail</small></div>
            </div>
            <div class="d-flex align-items-center gap-2">
                <select class="tme-rows-sel" id="rows-{{ $tp }}" onchange="OVData.reloadAll()"><option value="10">Top 10</option><option value="20">Top 20</option><option value="50">Top 50</option><option value="100" selected>Top 100</option></select>
...
<div class="do-panel" id="ovSntPanel"><div class="do-panel-header"><div class="do-panel-dot" id="ovPanelDot" style="background:var(--primary);"></div><span class="do-panel-title" id="ovPanelTitle">X Tweets</span><button class="do-panel-close" onclick="OVPanel.close()"><i class="ph ph-x"></i></button></div><div class="do-panel-actions"><div class="do-panel-meta"><i class="ph ph-magnifying-glass" style="font-size:11px;"></i><span id="ovPanelMeta">—</span></div></div><div class="do-panel-list" id="ovPanelList"></div><div class="do-detail-panel" id="ovDetailPanel"><div class="do-dp2-header"><button class="do-dp2-back" onclick="OVDetail.close()"><i class="ph ph-caret-left"></i></button><span class="do-dp2-title" id="ovDetailTitle">Detail</span><button class="do-panel-close" onclick="OVPanel.close()"><i class="ph ph-x"></i></button></div><div class="do-dp2-body" id="ovDetailBody"></div></div></div>
@endsection
@section('scripts')
...
<script>
'use strict';
const PLAT='X',API_BASE='/mk/api/x';
const OVCfg={pid:OV_PID,sd:OV_SD,ed:OV_ED,primary:'#038047',colors:{view:'#038047',retweet:'#10B981',like:'#F59E0B',reply:'#06B6D4',hashtag:'#038047'},perPage:10};
const DONUT_COLORS=['#038047','#273B4A','#F59E0B','#06B6D4','#EF4444'];
const TAB_TYPES=['hashtag','view','retweet','like','reply'];
const _$=id=>document.getElementById(id);const numF=n=>parseInt(n||0).toLocaleString('id-ID');const numK=n=>{n=parseInt(n||0);return n>=1e6?(n/1e6).toFixed(1)+'M':n>=1000?(n/1000).toFixed(1)+'k':String(n)};const esc=s=>(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');const dec=s=>{if(!s)return '';try{const f=decodeURIComponent(escape(s));if(!f.includes('\uFFFD')&&f!==s)return f}catch(e){}return s};const hideLd=id=>{const e=_$(id);if(e&&e.classList.contains('chart-loading'))e.classList.add('hidden')};
const Store={view:[],retweet:[],like:[],reply:[],hashtag:[]};const Pag={view:1,retweet:1,like:1,reply:1,hashtag:1};let allPostsRaw=[];let _engFetched=false;
const Charts={};function makeChart(id,opts){if(Charts[id]){try{Charts[id].destroy()}catch(e){}}const el=_$(id);if(!el)return null;Charts[id]=new ApexCharts(el,opts);Charts[id].render();el.style.display='block';return Charts[id]}window.addEventListener('resize',()=>Object.values(Charts).forEach(c=>{try{c.updateOptions({})}catch(e){}}));
/* ── TABS ── */
const OVTab={_loaded:{hashtag:false,view:false,retweet:false,like:false,reply:false},show(t){TAB_TYPES.forEach(x=>{_$('tab-'+x)?.classList.toggle('active',x===t);_$('panel-'+x)?.classList.toggle('active',x===t)});if(!this._loaded[t]){this._loaded[t]=true;OVData.loadTab(t)}else if(t!=='hashtag'&&Store[t].length)OVData._renderDonut(t,Store[t]);if(t!=='hashtag')setTimeout(()=>{_$('ovTabsBar')?.scrollIntoView({behavior:'smooth',block:'nearest'})},80)},reset(){this._loaded={hashtag:false,view:false,retweet:false,like:false,reply:false}}};
/* ── DATA ── */
const OVData={
    async loadTab(t){if(!OVCfg.pid){this._noProject(t);return}if(t==='hashtag'){await this.loadHashtags();return}await this._ensureEngagement();this._sortAndDisplay(t)},
    async loadHashtags(){const ld=_$('hashtagLoading'),ct=_$('hashtagContent'),em=_$('hashtagEmpty'),bd=_$('badgeHashtag');try{const r=await fetch(`${API_BASE}/trending-topics?start_date=${OVCfg.sd}&end_date=${OVCfg.ed}&location=Indonesia`);const j=await r.json();const topics=(j.data?.top_topics||[]).map(t=>({name:t.name,size:t.total_volume||t.appearances||0,url:t.url||''}));if(topics.length){Store.hashtag=topics;Pag.hashtag=1;if(bd)bd.textContent=topics.length+' topics';const ch=_$('chip-hashtag');if(ch)ch.textContent=topics.length;this._renderHashtagList(topics);this._renderDonutHashtag(topics)}else{if(ld)ld.style.display='none';if(em)em.style.display='flex';if(bd)bd.textContent='0'}}catch(e){console.error('[OV]',e);if(ld)ld.style.display='none';if(em)em.style.display='flex';if(bd)bd.textContent='Error'}},
    _renderHashtagList(hashtags){const ld=_$('hashtagLoading'),ct=_$('hashtagContent'),list=_$('hashtagList'),pg=_$('pag-hashtag');if(!list)return;const all=hashtags.slice(0,50);Store.hashtag=all;const page=Pag.hashtag||1,pp=OVCfg.perPage,total=all.length,pages=Math.ceil(total/pp),start=(page-1)*pp,items=all.slice(start,start+pp),mx=all[0]?.size||1;list.innerHTML='';items.forEach((h,i)=>{const rk=start+i+1,rc=rk<=3?` ht-rank--${rk}`:'',pct=Math.round((h.size/mx)*100);const el=document.createElement('div');el.className='ht-item';el.innerHTML=`<div class="ht-rank${rc}">${rk}</div><div class="ht-name">${esc(h.name)}</div><div class="ht-bar-wrap"><div class="ht-bar-fill" style="width:${pct}%;"></div></div><div class="ht-count">${numF(h.size)}</div>`;el.onclick=()=>this._openHashtagPanel(h);list.appendChild(el)});if(pg){if(pages<=1)pg.innerHTML='';else{const fr=start+1,to=Math.min(start+pp,total);let b='',r=2;b+=`<button class="tme-pag-btn" ${page<=1?'disabled':''} onclick="OVData.goHashtagPage(${page-1})"><i class="ph ph-caret-left"></i></button>`;for(let i=1;i<=pages;i++){if(i===1||i===pages||(i>=page-r&&i<=page+r))b+=`<button class="tme-pag-btn${i===page?' is-active':''}" onclick="OVData.goHashtagPage(${i})">${i}</button>`;else if(i===page-r-1||i===page+r+1)b+=`<span class="tme-pag-btn" style="cursor:default;opacity:.4;">…</span>`}b+=`<button class="tme-pag-btn" ${page>=pages?'disabled':''} onclick="OVData.goHashtagPage(${page+1})"><i class="ph ph-caret-right"></i></button>`;pg.innerHTML=`<div class="tme-pagination"><span class="tme-pag-info">Menampilkan ${fr}–${to} dari ${total} topics</span><div class="tme-pag-controls">${b}</div></div>`}}if(ld)ld.style.display='none';if(ct)ct.style.display='block'},
    ...
    _renderDonutHashtag(h){const ld=_$('loadingDonutHashtag'),ch=_$('donutHashtagChart');if(!ld||!ch||!h.length){hideLd('loadingDonutHashtag');return}this._renderDonutEcharts(ch,ld,_$('donutHashtagLegend'),h.slice(0,5),'mentions',x=>x.name,x=>x.size,x=>this._openHashtagPanel(x),'Topic','__ec_donutHashtagChart')},
    async _ensureEngagement(){if(_engFetched)return;const rows=parseInt(_$('rows-view')?.value||'100');['view','retweet','like','reply'].forEach(t=>{const ls=_$('list-'+t);if(ls)ls.innerHTML=`<div class="spinner-state"><div class="spin-ring"></div>Memuat data…</div>`});try{const r=await fetch(`${API_BASE}/most-engagement?project_id=${OVCfg.pid}&start_date=${OVCfg.sd}&end_date=${OVCfg.ed}&rows=${rows}`);const j=await r.json();let items=j.data||j||[];if(!Array.isArray(items))items=[];allPostsRaw=items;_engFetched=true;this._updateKpi(items);this._distributeItems(items)}catch(e){console.error('[OV]',e);['view','retweet','like','reply'].forEach(t=>{const ls=_$('list-'+t);if(ls)ls.innerHTML=`<div class="chart-empty" style="padding:40px;"><i class="ph ph-warning"></i><span>Gagal memuat: ${esc(e.message)}</span></div>`;hideLd('loadingDonut-'+t)})}},
    _distributeItems(items){const sortKey={view:'view_cnt',retweet:'rt',like:'fav_count',reply:'reply_cnt'};['view','retweet','like','reply'].forEach(t=>{const k=sortKey[t];Store[t]=[...items].sort((a,b)=>parseInt(b[k]||0)-parseInt(a[k]||0));Pag[t]=1;const ch=_$('chip-'+t);if(ch)ch.textContent=Store[t].length;const bd=_$('badge-'+t);if(bd)bd.textContent=Store[t].length+' tweets'})},
    _sortAndDisplay(t){this._renderList(t);this._renderDonut(t,Store[t])},
    _updateKpi(items){let tv=0,tr=0,tl=0,trp=0;items.forEach(i=>{tv+=parseInt(i.view_cnt||0);tr+=parseInt(i.rt||0);tl+=parseInt(i.fav_count||0);trp+=parseInt(i.reply_cnt||0)});const n=items.length,av=v=>n?Math.round(v/n):0;const el=(id,v)=>{const e=_$(id);if(e)e.textContent=numF(v)};const sub=(id,v,l)=>{const e=_$(id);if(e)e.innerHTML=`<i class="ph ph-chart-line-up me-1"></i>Avg ${numF(av(v))} / tweet &middot; ${n} tweets`};el('kpiViews',tv);sub('kpiViewsSub',tv);el('kpiRt',tr);sub('kpiRtSub',tr);el('kpiLikes',tl);sub('kpiLikesSub',tl);el('kpiReplies',trp);sub('kpiRepliesSub',trp)},
    reloadAll(){_engFetched=false;Store.view=[];Store.retweet=[];Store.like=[];Store.reply=[];Pag.view=1;Pag.retweet=1;Pag.like=1;Pag.reply=1;const activeTab=TAB_TYPES.find(t=>_$('tab-'+t)?.classList.contains('active'))||'view';this._ensureEngagement().then(()=>{['view','retweet','like','reply'].forEach(t=>{if(OVTab._loaded[t])this._sortAndDisplay(t)})})},
    ...
    _metric(item,type){const k={view:'view_cnt',retweet:'rt',like:'fav_count',reply:'reply_cnt'};return parseInt(item[k[type]]||0)},
    ...
    _getColor(item){const seed=item.sub_id||item.id||this._getName(item);const pal=['#038047','#273B4A','#F59E0B','#06B6D4','#8b5cf6','#ec4899','#f97316','#14b8a6'];let h=0;for(let i=0;i<seed.length;i++)h=(h*31+seed.charCodeAt(i))&0xffffffff;return pal[Math.abs(h)%pal.length]},
    ...
    _renderList(type){const items=Store[type],ls=_$('list-'+type),pg=_$('pag-'+type);if(!ls)return;if(!items.length){ls.innerHTML=`<div class="chart-empty" style="padding:40px;"><i class="ph ph-folder-open"></i><span>Tidak ada data untuk periode ini</span></div>`;if(pg)pg.innerHTML='';return}const page=Pag[type]||1,total=items.length,pp=OVCfg.perPage,pages=Math.ceil(total/pp),start=(page-1)*pp;ls.innerHTML=`<div class="tme-post-list">${items.slice(start,start+pp).map((it,i)=>this._postHtml(it,start+i,type)).join('')}</div>`;if(pg)pg.innerHTML=this._pagHtml(type,page,pages,total,start+1,Math.min(start+pp,total));ls.querySelectorAll('.tme-post').forEach(el=>{el.addEventListener('click',()=>{try{const it=JSON.parse(decodeURIComponent(el.dataset.item));const lm={view:'Most Viewed',retweet:'Most Retweeted',like:'Most Liked',reply:'Most Replies'};OVPanel.open(items,type,PLAT+' — '+lm[type]);OVDetail.open(it,type)}catch(e){console.warn(e)}})})},
    _pagHtml(type,page,pages,total,from,to){if(pages<=1)return '';let b='',r=2;b+=`<button class="tme-pag-btn" ${page<=1?'disabled':''} onclick="OVData.goPage('${type}',${page-1})"><i class="ph ph-caret-left"></i></button>`;for(let i=1;i<=pages;i++){if(i===1||i===pages||(i>=page-r&&i<=page+r))b+=`<button class="tme-pag-btn${i===page?' is-active':''}" onclick="OVData.goPage('${type}',${i})">${i}</button>`;else if(i===page-r-1||i===page+r+1)b+=`<span class="tme-pag-btn" style="cursor:default;opacity:.4;">…</span>`}b+=`<button class="tme-pag-btn" ${page>=pages?'disabled':''} onclick="OVData.goPage('${type}',${page+1})"><i class="ph ph-caret-right"></i></button>`;return `<div class="tme-pagination"><span class="tme-pag-info">Menampilkan ${from}–${to} dari ${total} tweets</span><div class="tme-pag-controls">${b}</div></div>`},
    goPage(t,p){Pag[t]=p;this._renderList(t);_$('list-'+t)?.scrollIntoView({behavior:'smooth',block:'nearest'})},
    _postHtml(item,gi,type){const rank=gi+1,rkC=rank<=3?'--'+rank:'',name=this._getName(item),scr=this._getScr(item),color=this._getColor(item),avH=this._avHtml(item),sent=this._normSent(item);const content=dec((item.content||'').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()).slice(0,200);const dt=(item.date_created||'').split('T')[0],url=this._tweetUrl(item);const v=parseInt(item.view_cnt||0),rt=parseInt(item.rt||0),l=parseInt(item.fav_count||0),rp=parseInt(item.reply_cnt||0),total=v+rt+l+rp;const sL={pos:'Positive',neg:'Negative',neu:'Neutral'}[sent],enc=encodeURIComponent(JSON.stringify(item));const vC=type==='view'?' tme-metric--primary':'',rtC=type==='retweet'?' tme-metric--primary':'',lC=type==='like'?' tme-metric--amber':'',rpC=type==='reply'?' tme-metric--cyan':'';const av=this._getAvatar(item),thH=(av&&av.startsWith('http'))?`<div class="tme-post-thumb"><img src="${esc(av)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'tme-post-thumb-ph\\'>🐦</div>'"></div>`:`<div class="tme-post-thumb"><div class="tme-post-thumb-ph">🐦</div></div>`;return `<div class="tme-post" data-item="${esc(enc)}"><div class="tme-post-rank tme-post-rank${rkC}">${rank}</div><div class="tme-post-av" style="background:linear-gradient(135deg,${color},${color}99);">${avH}</div><div class="tme-post-body"><div class="tme-post-author">${esc(name)}${scr?` <span style="color:var(--slate-400);font-weight:500;">@${esc(scr)}</span>`:''}</div>${dt?`<div class="tme-post-date">${dt}</div>`:''}${content?`<div class="tme-post-text">${esc(content)}</div>`:''}<div class="tme-post-stats"><span class="tme-metric${vC}"><i class="ph ph-eye me-1"></i>${numF(v)}</span><span class="tme-metric${rtC}"><i class="ph ph-repeat me-1"></i>${numF(rt)}</span><span class="tme-metric" style="font-weight:800;">∑ ${numF(total)}</span><span class="tme-sent tme-sent--${sent}">${sL}</span>${url?`<a href="${esc(url)}" target="_blank" rel="noopener" class="tme-view-link" onclick="event.stopPropagation()"><i class="ph ph-arrow-square-out me-1"></i>Lihat</a>`:''}</div></div>${thH}</div>`},
    _renderDonut(type,items){const ld=_$('loadingDonut-'+type),ch=_$('donutChart-'+type),em=_$('donutEmpty-'+type),lg=_$('donutLegend-'+type);if(!ld||!ch)return;if(!items.length){ld.style.display='none';if(em)em.style.display='flex';return}const top5=items.slice(0,5),ml={view:'Views',retweet:'Retweets',like:'Likes',reply:'Replies'}[type];this._renderDonutEcharts(ch,ld,lg,top5,ml,it=>this._getName(it),it=>this._metric(it,type),it=>{OVPanel.open(Store[type],type,PLAT);OVDetail.open(it,type)},ml,it=>dec((it.content||'').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()),'__ec_donutChart_'+type)},
    _renderDonutEcharts(chartEl,loadEl,legEl,data,metricLabel,getNameFn,getValFn,onClickFn,titleLabel,getDescFn,windowKey){if(!chartEl||!data.length){if(loadEl)loadEl.style.display='none';return}const total=data.reduce((s,d)=>s+getValFn(d),0);if(legEl)legEl.innerHTML=data.map((d,i)=>{const n=getNameFn(d),sn=n.length>22?n.slice(0,21)+'…':n;return `<div class="donut-leg-item"><span class="donut-dot" style="background:${DONUT_COLORS[i]};"></span>${sn} · ${numF(getValFn(d))}</div>`}).join('');if(loadEl)loadEl.style.display='none';chartEl.style.display='block';const ek=windowKey||('__ec_'+chartEl.id);if(window[ek]){try{window[ek].dispose()}catch(e){}}if(typeof echarts==='undefined'){chartEl.innerHTML='<div class="chart-empty"><i class="ph ph-chart-donut"></i><span>ECharts not loaded</span></div>';return}const chart=echarts.init(chartEl,null,{renderer:'canvas'});window[ek]=chart;window.addEventListener('resize',()=>{try{chart.resize()}catch(e){}});const pd=data.map((d,i)=>({name:getNameFn(d),value:getValFn(d),_desc:getDescFn?getDescFn(d):'',itemStyle:{color:DONUT_COLORS[i]}}));chart.setOption({backgroundColor:'transparent',animation:true,animationDuration:1000,animationEasing:'cubicOut',animationDelay:idx=>idx*80,tooltip:{show:false},series:[{type:'pie',radius:['38%','62%'],center:['50%','50%'],avoidLabelOverlap:true,selectedMode:false,minAngle:8,itemStyle:{borderColor:'#fff',borderWidth:3},label:{show:true,position:'outside',alignTo:'edge',edgeDistance:20,lineHeight:18,fontSize:11,fontFamily:'inherit',color:'#334155',fontWeight:'500',formatter:p=>{const d=data[p.dataIndex],desc=getDescFn?getDescFn(d).slice(0,80):'',words=desc?desc.split(' '):[],lines=[];let cur='';words.forEach(w=>{if((cur+' '+w).trim().length>40){lines.push(cur.trim());cur=w}else cur=(cur+' '+w).trim()});if(cur)lines.push(cur);const body=lines.join('\n');return `{title|${p.name}}\n${body?body+'\n':''}({val|${numF(p.value)}} ${metricLabel}, {pct|${p.percent.toFixed(1)}%})`},rich:{title:{fontSize:11,fontWeight:'700',color:'#1e293b',lineHeight:18},val:{fontSize:11,fontWeight:'700',color:'#038047'},pct:{fontSize:11,fontWeight:'600',color:'#64748b'}}},labelLine:{show:true,length:18,length2:24,smooth:0.3,lineStyle:{width:1.5,color:'#94A3B8'}},emphasis:{scale:false,itemStyle:{shadowBlur:0,shadowColor:'transparent',borderWidth:3,borderColor:'#fff',opacity:1},labelLine:{lineStyle:{width:2.5,color:'#273B4A'}},label:{show:true}},select:{disabled:true},data:pd}],graphic:[{type:'text',left:'center',top:'46%',z:100,style:{text:numK(total),fill:'#0f172a',font:"800 28px inherit",textAlign:'center'}},{type:'text',left:'center',top:'54%',z:100,style:{text:'TOTAL '+titleLabel.toUpperCase(),fill:'#94a3b8',font:"600 9px inherit",textAlign:'center'}}]});chart.on('click',p=>{const d=data[p.dataIndex];if(d&&onClickFn)onClickFn(d)});let _tt=document.getElementById('ovCustomTT');if(!_tt){_tt=document.createElement('div');_tt.id='ovCustomTT';_tt.style.cssText='position:fixed;z-index:9999;pointer-events:none;background:#1e293b;color:#fff;border:1px solid #334155;border-radius:6px;padding:10px 14px;max-width:280px;font-size:12px;line-height:1.5;display:none;box-shadow:0 8px 24px rgba(0,0,0,.32);font-family:inherit;opacity:0;transform:translateY(6px) scale(.97);transition:opacity .18s ease,transform .18s ease;';document.body.appendChild(_tt)}let _tm=null;chart.on('mouseover',p=>{if(p.componentType!=='series')return;const d=data[p.dataIndex],cl=DONUT_COLORS[p.dataIndex],desc=(getDescFn?getDescFn(d):'').slice(0,160);clearTimeout(_tm);_tt.innerHTML=`<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;"><span style="width:9px;height:9px;border-radius:50%;background:${cl};flex-shrink:0;display:inline-block;"></span><b style="font-size:12.5px;">${esc(p.name)}</b></div>${desc?`<div style="font-size:11px;color:#94a3b8;margin-bottom:6px;">${esc(desc)}</div>`:''}<div style="display:flex;align-items:center;gap:8px;"><b style="font-size:13px;">${numF(p.value)} ${metricLabel}</b><span style="color:${cl};font-weight:700;">${p.percent.toFixed(1)}%</span></div>`;_tt.style.display='block';requestAnimationFrame(()=>{_tt.style.opacity='1';_tt.style.transform='translateY(0) scale(1)'})});chart.on('mouseout',()=>{_tt.style.opacity='0';_tt.style.transform='translateY(6px) scale(.97)';_tm=setTimeout(()=>{_tt.style.display='none'},180)});chartEl.addEventListener('mousemove',e=>{if(_tt.style.display==='none')return;const vw=window.innerWidth,vh=window.innerHeight,tw=_tt.offsetWidth+16,th=_tt.offsetHeight+16;let x=e.clientX+18,y=e.clientY-10;if(x+tw>vw)x=e.clientX-tw;if(y+th>vh)y=e.clientY-th;_tt.style.left=x+'px';_tt.style.top=y+'px'})},
    _openHashtagPanel(h){const tag=h.name.toLowerCase();const filtered=allPostsRaw.filter(p=>(p.content||'').toLowerCase().includes(tag));OVPanel.open(filtered.length?filtered:allPostsRaw.slice(0,20),'view',`${PLAT} — ${h.name} (${h.size} mentions)`)},
};
/* ── PANEL ── */
const OVPanel={_items:[],_type:null,open(items,type,title){this._items=items||[];this._type=type;OVDetail.close();_$('ovPanelDot').style.background=OVCfg.colors[type]||OVCfg.primary;_$('ovPanelTitle').textContent=title||PLAT+' Tweets';_$('ovPanelMeta').textContent=OVCfg.sd+' – '+OVCfg.ed;const ov=_$('ovPanelOverlay'),pn=_$('ovSntPanel');ov.classList.remove('hiding');pn.classList.remove('hiding');ov.classList.add('show');pn.classList.add('show');this._render(items,type)},close(){OVDetail.close();const ov=_$('ovPanelOverlay'),pn=_$('ovSntPanel');pn.classList.add('hiding');ov.classList.add('hiding');setTimeout(()=>{pn.classList.remove('show','hiding');ov.classList.remove('show','hiding')},240)},_render(items,type){const list=_$('ovPanelList');if(!list)return;if(!items?.length){list.innerHTML='<div class="do-panel-loading"><div class="do-panel-spinner"></div>Tidak ada data</div>';return}const ml={view:'Views',retweet:'Retweets',like:'Likes',reply:'Replies'}[type]||'Views';list.innerHTML=items.slice(0,100).map(item=>{const nm=OVData._getName(item),av=OVData._getAvatar(item),cl=OVData._getColor(item),d='/assets/images/user/dummy.jpg',aH=(av&&av.startsWith('http'))?`<img src="${esc(av)}" onerror="this.src='${d}'">`:`<img src="${d}">`,tx=(item.content||'').replace(/<[^>]*>/g,'').trim(),mv=OVData._metric(item,type),dt=(item.date_created||'').split('T')[0],sn=OVData._normSent(item),sl={pos:'Pos',neg:'Neg',neu:'Neu'}[sn],v=parseInt(item.view_cnt||0),rt=parseInt(item.rt||0),l=parseInt(item.fav_count||0),rp=parseInt(item.reply_cnt||0),total=v+rt+l+rp,enc=encodeURIComponent(JSON.stringify(item));return `<div class="do-panel-item" data-item="${esc(enc)}" data-type="${type}" onclick="OVPanel._click(this)"><div class="do-panel-avatar" style="background:linear-gradient(135deg,${cl},${cl}99);">${aH}</div><div class="do-panel-item-body"><div class="do-panel-author">${esc(nm)}</div><div class="do-panel-text">${esc(dec(tx).slice(0,130)||'(tidak ada konten)')}</div><div class="do-panel-footer"><span class="do-sent-badge do-sent-badge--${sn}">${sl}</span><span>${ml} ${numF(mv)}</span><span>∑ ${numF(total)}</span>${dt?`<span style="margin-left:auto;">${dt}</span>`:''}</div></div></div>`}).join('');if(items.length>100)list.insertAdjacentHTML('beforeend',`<div style="padding:8px;text-align:center;font-size:10px;font-weight:600;color:#94A3B8;background:var(--slate-50);border-top:1px dashed var(--slate-200);">+${(items.length-100).toLocaleString()} lainnya</div>`)},_click(el){try{const item=JSON.parse(decodeURIComponent(el.dataset.item.replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"')));OVDetail.open(item,el.dataset.type||this._type)}catch(e){console.warn(e)}}};
/* ── DETAIL ── */
const OVDetail={open(item,type){const panel=_$('ovDetailPanel'),body=_$('ovDetailBody'),title=_$('ovDetailTitle');if(!panel||!body)return;const color=OVCfg.colors[type]||OVCfg.primary,name=OVData._getName(item),scr=OVData._getScr(item),av=OVData._getAvatar(item),ac=OVData._getColor(item),d='/assets/images/user/dummy.jpg',aH=(av&&av.startsWith('http'))?`<img src="${esc(av)}" onerror="this.src='${d}'">`:`<img src="${d}">`;const raw=(item.content||'').replace(/<[^>]*>/g,'').trim(),ct=raw?dec(raw):'',url=OVData._tweetUrl(item),dt=item.date_created||'';const v=parseInt(item.view_cnt||0),rt=parseInt(item.rt||0),l=parseInt(item.fav_count||0),rp=parseInt(item.reply_cnt||0);const sn=OVData._normSent(item),sl={pos:'Positif',neg:'Negatif',neu:'Netral'}[sn];let dtF='';if(dt)try{dtF=new Date(dt).toLocaleDateString('id-ID',{weekday:'long',day:'2-digit',month:'long',year:'numeric',hour:'2-digit',minute:'2-digit'})}catch(e){dtF=dt.split('T')[0]}const flw=item.author?.flw_cnt;title.textContent=name;body.innerHTML=`<div class="do-dp2-avatar-row"><div class="do-dp2-avatar-lg" style="background:linear-gradient(135deg,${ac},${ac}99);">${aH}</div><div><div class="do-dp2-name">${esc(name)}</div>${scr?`<div class="do-dp2-handle">@${esc(scr)}</div>`:''}<span class="do-dp2-plat-badge" style="background:${color}18;color:${color};">X / Twitter</span>${flw?`<div style="font-size:10px;color:var(--slate-400);margin-top:3px;"><i class="ph ph-users me-1"></i>${numF(flw)} followers</div>`:''}</div></div>${dtF?`<div class="do-dp2-meta">${dtF}</div>`:''}<div class="do-dp2-sent do-dp2-sent--${sn}">${sl}</div>${ct?`<div class="do-dp2-content">${esc(ct)}</div>`:''}<div class="do-dp2-stats"><div class="do-dp2-stat"><div class="do-dp2-stat-val">${numF(v)}</div><div class="do-dp2-stat-lbl">Views</div></div><div class="do-dp2-stat"><div class="do-dp2-stat-val">${numF(rt)}</div><div class="do-dp2-stat-lbl">Retweets</div></div><div class="do-dp2-stat"><div class="do-dp2-stat-val">${numF(l)}</div><div class="do-dp2-stat-lbl">Likes</div></div><div class="do-dp2-stat"><div class="do-dp2-stat-val">${numF(rp)}</div><div class="do-dp2-stat-lbl">Replies</div></div></div>${url?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer" class="do-dp2-link"><i class="ph ph-arrow-square-out me-1"></i>Buka di X</a>`:''}`;panel.classList.add('show')},close(){_$('ovDetailPanel')?.classList.remove('show')}};
...
const OVExport = (() => {
    ...
    /* ── Collect all active ECharts instances ── */
    function _resizeAllCharts() {
        /* ECharts donut instances */
        const ecKeys = [
            '__ec_donutHashtagChart',
            '__ec_donutChart_view',
            '__ec_donutChart_retweet',
            '__ec_donutChart_like',
            '__ec_donutChart_reply',
        ];
        ecKeys.forEach(k => { if(window[k]) { try{ window[k].resize(); }catch(e){} } });
        /* ApexCharts */
    ...
    async function _exportPdf() {
        ...
        const _drawHeader = (doc) => {
            ...
            doc.text('SMADIMENT — X Overview', margin, 7.5);
    ...
    async function _captureCard(areaId, cardKey) {
        ...
        /* Resize relevant ECharts */
        const ecMap = {
            'hashtag-donut' : '__ec_donutHashtagChart',
            'donut-view'    : '__ec_donutChart_view',
            'donut-retweet' : '__ec_donutChart_retweet',
            'donut-like'    : '__ec_donutChart_like',
            'donut-reply'   : '__ec_donutChart_reply',
        };
        if(ecMap[cardKey] && window[ecMap[cardKey]]) {
            try{ window[ecMap[cardKey]].resize(); }catch(e){}
        }
    ...
    function _cardFilename(cardKey) {
        const labels = {
            'hashtag-list'   : 'top-topics-list',
            'hashtag-donut'  : 'top-topics-donut',
            'donut-view'     : 'distribusi-most-viewed',
            'donut-retweet'  : 'distribusi-most-retweeted',
            'donut-like'     : 'distribusi-most-liked',
            'donut-reply'    : 'distribusi-most-replies',
            'list-view'      : 'top-tweets-by-view',
            'list-retweet'   : 'top-tweets-by-retweet',
            'list-like'      : 'top-tweets-by-like',
            'list-reply'     : 'top-tweets-by-reply',
        };
        const stamp = new Date().toISOString().slice(0,10).replace(/-/g,'');
        return `x_overview_${labels[cardKey]||cardKey}_${OV_PID}_${stamp}`;
    }

    /* ── Per-card PDF label map ── */
    function _cardLabel(cardKey) {
        const m = {
            'hashtag-list'  : 'Top Topics / Hashtags',
            'hashtag-donut' : 'Distribusi Top Topics',
            'donut-view'    : 'Distribusi Most Viewed',
            'donut-retweet' : 'Distribusi Most Retweeted',
            'donut-like'    : 'Distribusi Most Liked',
            'donut-reply'   : 'Distribusi Most Replies',
            'list-view'     : 'Top Tweets by Views',
            'list-retweet'  : 'Top Tweets by Retweets',
            'list-like'     : 'Top Tweets by Likes',
            'list-reply'    : 'Top Tweets by Replies',
        };
...
/* ── INIT ── */
document.addEventListener('DOMContentLoaded',()=>{if(!OVCfg.pid)return;OVTab._loaded.hashtag=true;OVData.loadHashtags();OVData._ensureEngagement().then(()=>{OVTab._loaded.view=true;OVData._sortAndDisplay('view')});document.addEventListener('keydown',e=>{if(e.key==='Escape')OVPanel.close()})});
</script>
@endsection