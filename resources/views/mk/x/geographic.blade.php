...
const XGeo = {
    ...
    renderMap(elementId, rows, getMarkerProps) {
        ...
        rows.forEach(p=>{
            ...
            const pin=L.marker([lat,lng],{icon:L.divIcon({className:'',html:`<div style="width:13px;height:13px;background:${color};border:2.5px solid #fff;border-radius:50%;box-shadow:0 2px 5px rgba(0,0,0,.4);"></div>`,iconSize:[13,13],iconAnchor:[6.5,6.5]})}).addTo(map).bindPopup(popup);
            markerRefs.push({marker:pin,lat,lng});
            const label=count>999?(count/1000).toFixed(1)+'k':String(count);
            L.marker([lat,lng],{icon:L.divIcon({className:'circle-label',html:`<div style="font-family:inherit;font-size:${count>=1000?'13px':'11px'};font-weight:900;color:#fff;background:${color};padding:3px 8px;border-radius:12px;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,.4);white-space:nowrap;">${label}</div>`,iconSize:[40,20],iconAnchor:[20,25]}),interactive:false}).addTo(map);
        });
