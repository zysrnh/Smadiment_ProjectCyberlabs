...
@section('styles')
  <style>
    ...
    .tm-modal-search:focus {
      ...
      box-shadow: 0 0 0 3px var(--tm-primary-lt);
    }
          ...
          <div class="d-flex align-items-center">
            <div class="flex-grow-1">
              ...
              <p class="mb-0 mt-2 text-white text-opacity-75 f-12">
                <i class="ph ph-circle-dashed me-1"></i>Unique topics
              </p>
            </div>
        ...
        <div class="card-body">
          <div class="d-flex align-items-center">
            <div class="flex-grow-1">
              ...
              <p class="mb-0 mt-2 text-white text-opacity-75 f-12">
                <i class="ph ph-trend-up me-1"></i>Most mentioned
              </p>
            </div>
            <div class="flex-shrink-0 ms-3">
        ...
        <div class="card-body">
          <div class="d-flex align-items-center">
            <div class="flex-grow-1">
              ...
              <p class="mb-0 mt-2 text-white text-opacity-75 f-12">
                <i class="ph ph-chat-dots me-1"></i>Across all topics
              </p>
            </div>
            <div class="flex-shrink-0 ms-3">
    ...
    const TMApp = (() => {
      ...
      function _renderBar(topics) {
        ...
        barChartInst = new Chart(ctx, {
          ...
          data: {
            ...
            datasets: [{ label: 'Mentions', data: top.map(t => t.count), backgroundColor: primary + 'CC', borderColor: primary, borderWidth: 2, borderRadius: 6 }]
          },
          options: {
      ...
      function _renderPie(topics) {
        ...
        pieChartInst = new Chart(ctx, {
          ...
          data: {
            labels,
            datasets: [{ data, backgroundColor: colors, borderColor: '#fff', borderWidth: 3, hoverOffset: 14 }]
          },
          options: {
            ...
            plugins: {
              legend: {
                position: 'right',
                labels: {
                  padding: 18, font: { size: 12, weight: '600', family: 'inherit' },
                  generateLabels: c => c.data.labels.map((l, i) => ({
                    text: `${l} (${nF(c.data.datasets[0].data[i])})`,
              ...
              tooltip: {
                ...
                titleFont: { size: 13, weight: 'bold', family: 'inherit' },
                bodyFont: { size: 12, family: 'inherit' },
                borderColor: '#4361EE', borderWidth: 1,
                callbacks: { label: c => { const tot = c.dataset.data.reduce((a, b) => a + b, 0); return `${c.label}: ${nF(c.parsed)} (${((c.parsed / tot) * 100).toFixed(1)}%)`; } }
              }
            }
          }
        });
      ...
      function _renderList(topics) {
        ...
        if (!topics.length) {
          ...
          viewAllWrap.style.display = 'none'; return;
        }
        listEl.innerHTML = topics.slice(0, 10).map((t, i) => {
          const rank = i + 1;
          return `<div class="topic-item">
                          <span class="topic-rank${rank <= 3 ? ' top-3' : ''}">#${rank}</span>
                          <span class="topic-name">${t.name}</span>
                          <span class="topic-count">${nF(t.count)}</span>
                      </div>`;
        }).join('');
      ...
      function openModal() {
        ...
        listEl.innerHTML = topicsData.map((t, i) => {
          const rank = i + 1;
          return `<div class="topic-item" data-name="${t.name.toLowerCase()}">
                          <span class="topic-rank${rank <= 3 ? ' top-3' : ''}">#${rank}</span>
                          <span class="topic-name">${t.name}</span>
                          <span class="topic-count">${nF(t.count)}</span>
                      </div>`;
      ...
      return { switchChart, openModal, closeModal, closeModalOnOverlay };
    })();
  </script>
@endsection