            ...
            <div class="card-body">
                <div class="d-flex align-items-center">
                    <div class="flex-grow-1">
                        ...
                        <h3 class="mb-0 text-white f-w-300" id="kpiPosts"><span class="sk-block" style="width:80px;height:24px;display:inline-block;"></span></h3>
    ...
    <div class="card mb-3" style="animation:fadeUp .38s ease-out .2s both;">
        <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <button class="btn btn-outline-secondary btn-sm" onclick="IMEData.exportCsv('{{ $tp }}')" title="Export CSV"><i class="ph ph-download-simple me-1"></i>CSV</button>
...
<div class="do-panel" id="imeSntPanel">
    ...
    <div class="do-panel-actions">
        ...
        <button class="do-panel-export" onclick="IMEPanel.exportCsv()"><i class="ph ph-download-simple"></i> CSV</button>
    </div>
...
const IMEData={
    ...
    exportCsv(type){
        ...
        const hdr='index;nama;sentiment;likes;comments;total_engagement;tipe;tanggal;url;konten';
        const rows=items.map((it,i)=>{const name=this._getName(it),sent=this._normSent(it),sentLbl={pos:'Positif',neg:'Negatif',neu:'Netral'}[sent];const l=parseInt(it.likes||0),c=parseInt(it.comments||0);const mType=this._getType(it);const content=(it.content||'').replace(/<[^>]*>/g,'').trim().slice(0,300).replace(/;/g,',').replace(/\n/g,' ');return`${i};${name.replace(/;/g,',')};${sentLbl};${l};${c};${l+c};${mType||'-'};${(it.date_created||'').split('T')[0]};${it.url||''};${content}`;});
        const blob=new Blob(['\uFEFF'+[hdr,...rows].join('\r\n')],{type:'text/csv;charset=utf-8;'});
        const a=Object.assign(document.createElement('a'),{href:URL.createObjectURL(blob),download:`Instagram_Most${type.charAt(0).toUpperCase()+type.slice(1)}_${IMECfg.sd}_${IMECfg.ed}.csv`});a.click();
    }
};
...
const IMEPanel={
    ...
    exportCsv(){IMEData.exportCsv(this._type||'like');},
    _render(items,type){
