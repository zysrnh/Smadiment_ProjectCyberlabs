...
.do-dp2-media img{width:100%;display:block;border-radius:var(--radius-sm)}
...
<div id="pageExportArea">
    ...
    <div class="row mb-3">
        <div class="col-md-6 col-xl-3">
            <div class="card h-100 text-white kpi-card-hover fade-up fade-up-d1"
                 style="background:linear-gradient(135deg,#e6683c,#dc2743);">
        ...
        <div class="col-md-6 col-xl-3">
            <div class="card h-100 text-white kpi-card-hover fade-up fade-up-d2"
                 style="background:linear-gradient(135deg,#cc2366,#bc1888);">
                <div class="card-body"><div class="d-flex align-items-center">
                    <div class="flex-grow-1">
    ...
    <div class="page-export-bar" data-html2canvas-ignore="true">
        ...
        <div class="page-export-bar-right">
            <button type="button" class="page-export-btn page-export-btn-pdf" id="pageExportPdfBtn"
                    onclick="IGExport.run('pdf',this)" title="Export PDF">
            ...
            <button type="button" class="page-export-btn page-export-btn-img" id="pageExportImgBtn"
                    onclick="IGExport.run('image',this)" title="Export PNG">
        ...
        <div class="card mb-3" style="animation:fadeUp .38s ease-out .22s both;">
            <div id="card-export-hashtag-donut">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <div class="d-flex gap-1" data-html2canvas-ignore="true">
                        <button class="card-exp-btn card-exp-btn-pdf" onclick="IGExport.runCard('card-export-hashtag-donut','hashtag-donut','pdf',this)" title="PDF"><i class="ph ph-file-pdf export-icon"></i><span class="export-spinner"></span></button>
                        <button class="card-exp-btn card-exp-btn-img" onclick="IGExport.runCard('card-export-hashtag-donut','hashtag-donut','image',this)" title="PNG"><i class="ph ph-image export-icon"></i><span class="export-spinner"></span></button>
                    </div>
                </div>
    ...
    {{-- ══ Panel: Most Liked / Most Comments ══ --}}
    @foreach(['like' => 'Most Liked', 'comment' => 'Most Comments'] as $tp => $tpLabel)
    @php
        ...
        <div class="card mb-3" style="animation:fadeUp .38s ease-out .18s both;">
            <div id="card-export-donut-{{ $tp }}">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        <div class="avtar avtar-xs bg-light-primary rounded"><i class="ph ph-chart-donut f-18 text-primary"></i></div>
                        <div><h6 class="mb-0">Distribusi {{ $tpLabel }} — Top 5</h6><small class="text-muted">Proporsi engagement per post — klik untuk detail</small></div>
                    </div>
                    <div class="d-flex gap-1" data-html2canvas-ignore="true">
                        <button class="card-exp-btn card-exp-btn-pdf" onclick="IGExport.runCard('card-export-donut-{{ $tp }}','donut-{{ $tp }}','pdf',this)" title="PDF"><i class="ph ph-file-pdf export-icon"></i><span class="export-spinner"></span></button>
                        <button class="card-exp-btn card-exp-btn-img" onclick="IGExport.runCard('card-export-donut-{{ $tp }}','donut-{{ $tp }}','image',this)" title="PNG"><i class="ph ph-image export-icon"></i><span class="export-spinner"></span></button>
                    </div>
                </div>
                <div class="card-body pb-0">
                    <div class="donut-stack-wrap">
            ...
            <div id="card-export-list-{{ $tp }}">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div><h6 class="mb-0">Top Posts by {{ $tpLabel }}</h6><small class="text-muted">Klik post untuk lihat detail</small></div>
                    </div>
                    <div class="d-flex align-items-center gap-2">
...
const API_BASE = '/mk/api/instagram';
const OVCfg    = {
    pid    : OV_PID,
    sd     : OV_SD,
    ed     : OV_ED,
...
const Store = { like:[], comment:[], hashtag:[] };
const Pag   = { like:1,  comment:1,  hashtag:1  };
let allPostsRaw = [];

window.addEventListener('resize', () => {
    ['__ec_donutHashtagChart','__ec_donutChart-like','__ec_donutChart-comment']
...
const OVData = {
    ...
    async loadHashtags() {
        ...
        try {
            ...
            if (j.success && j.data?.hashtags?.length) {
                ...
                if(ld) ld.style.display='none'; if(em) em.style.display='flex'; if(bd) bd.textContent='0';
            }
        } catch(e) {
            console.error('[OV]',e);
            if(ld) ld.style.display='none'; if(em) em.style.display='flex'; if(bd) bd.textContent='Error';
        }
    },
    ...
    async loadEngagement(type) {
        ...
        try {
            ...
            const bd=_$('badge-'+type); if(bd) bd.textContent=items.length+' posts';
            if(type==='like' && !allPostsRaw.length) allPostsRaw=items;
            if(type==='like') this._updateKpi(items);
    ...
    _getName(item) { const n=(item.name||item.author_scr_name||item.author_name||'').replace(/<[^>]*>/g,'').trim(); return n||'Instagram User'; },
    ...
    _getColor(item) {
        ...
        const pal=['#e6683c','#dc2743','#cc2366','#bc1888','#038047','#273B4A','#F59E0B','#06B6D4'];
        ...
        ls.querySelectorAll('.tme-post').forEach(el => {
            el.addEventListener('click', () => {
                try {
                    ...
                    const lm={like:'Most Liked',comment:'Most Comments'};
    ...
    _pagHtml(type,page,pages,total,from,to) {
        ...
        return `<div class="tme-pagination"><span class="tme-pag-info">Menampilkan ${from}–${to} dari ${total} posts</span><div class="tme-pag-controls">${b}</div></div>`;
    },
    ...
    _postHtml(item,gi,type) {
        ...
        const l=parseInt(item.num_likes||item.likes||0), c=parseInt(item.num_comments||item.comments||0), total=l+c;
        const sL={pos:'Positive',neg:'Negative',neu:'Neutral'}[sent];
        const enc=encodeURIComponent(JSON.stringify(item));
        const lC=type==='like'?' tme-metric--primary':'', cC=type==='comment'?' tme-metric--amber':'';
    ...
    _renderDonut(type, items) {
        ...
        const top5=items.slice(0,5), ml={like:'Likes',comment:'Comments'}[type];
    ...
    /* ── Breakdown list di bawah donut (identik X/YT/FB) ── */
    ...
    /* ── ECharts donut (identik X/YT/FB: label ringkas, breakdown terpisah) ── */
    ...
    _openHashtagPanel(h) {
        ...
        const filtered=allPostsRaw.filter(p=>(p.content||p.caption||'').toLowerCase().includes(tag));
        OVPanel.open(filtered.length?filtered:allPostsRaw.slice(0,20), 'like', `${PLAT} — #${h.name} (${h.size} mentions)`);
    },
};
...
const OVPanel = {
    ...
    open(items, type, title) {
        ...
        _$('ovPanelTitle').textContent=title||PLAT+' Posts';
    ...
    _render(items,type) {
        ...
        list.innerHTML=items.slice(0,100).map(item=>{
            const nm=OVData._getName(item), av=OVData._getAvatar(item), cl=OVData._getColor(item),
                  ...
                  tx=(item.content||item.caption||'').replace(/<[^>]*>/g,'').trim(),
                  mv=OVData._metric(item,type), dt=(item.date_created||'').split('T')[0],
                  sn=OVData._normSent(item), sl={pos:'Pos',neg:'Neg',neu:'Neu'}[sn],
                  l=parseInt(item.num_likes||item.likes||0), c=parseInt(item.num_comments||item.comments||0),
                  total=l+c, enc=encodeURIComponent(JSON.stringify(item));
...
const OVDetail = {
    open(item,type) {
        ...
        const raw=(item.content||item.caption||'').replace(/<[^>]*>/g,'').trim(),
              ct=raw?dec(raw):'', url=item.url||item.link||'', dt=item.date_created||'';
        const l=parseInt(item.num_likes||item.likes||0), c=parseInt(item.num_comments||item.comments||0);
        ...
        const mH=imgUrl?`<div class="do-dp2-media"><img src="${esc(imgUrl)}" alt="post" onerror="this.parentElement.style.display='none'"></div>`:'';
        ...
        body.innerHTML=`<div class="do-dp2-avatar-row">
            ...
            <div>
                ...
                <span class="do-dp2-plat-badge" style="background:#e6683c18;color:#e6683c;">Instagram</span>
            </div>
        </div>
        ...
        ${url?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer" class="do-dp2-link"><i class="ph ph-arrow-square-out me-1"></i>Buka di Instagram</a>`:''}`;
...
/* ════════════════════════════════════════════════════════
   IGExport — IG Overview (identik X/YT/FB: _snapshotForTab + skipSnapshot)
════════════════════════════════════════════════════════ */
const IGExport = (() => {
    'use strict';
    ...
    function _freeze(){
        ...
        const s=document.createElement('style'); s.id='__ig_freeze';
    ...
    function _snapshotForTab(tabKey) {
        ...
        (TAB_EC_MAP[tabKey]||[]).forEach(([key,containerId]) => {
            ...
            } catch(e){ console.warn('[IGExport] snapshot tab failed:',key,e); }
        });
    }
    ...
    function _onClone(clonedDoc) {
        ...
        clonedDoc.querySelectorAll('.tme-post-av,.do-panel-avatar,.do-dp2-avatar-lg').forEach(wrapper => {
            ...
            if(!wrapper.querySelector('.__ini')){
                const txt=(wrapper.textContent||'').trim(), initial=txt?txt[0].toUpperCase():'I';
            ...
            if(!wrapper.style.background) wrapper.style.background='linear-gradient(135deg,#e6683c,#cc2366)';
        });
        /* Sembunyikan image post di detail (cross-origin) */
        clonedDoc.querySelectorAll('.do-dp2-media img').forEach(img => { img.style.display='none'; });
    ...
    function _drawHeader(pdf, pW, pH, label, page, total) {
        ...
        pdf.text('SMADIMENT — '+(label||'Instagram Overview'), 10, 7.5);
        ...
        try {
            ...
            canvases.forEach(({label,canvas})=>{
                ...
                while(srcY<canvas.height){
                    ...
                    _drawHeader(pdf, pW, pH, `Instagram Overview — ${label}`, pageNum, totalPages);
            ...
            console.error('[IGExport.run]',err); _toast('Export gagal: '+err.message,'error');
        } finally {
    ...
    function _cardFilename(k){
        const map={
            ...
            'donut-like':'distribusi-most-liked','donut-comment':'distribusi-most-comments',
            'list-like':'top-posts-by-like','list-comment':'top-posts-by-comment',
        };
        const stamp=new Date().toISOString().slice(0,10).replace(/-/g,'');
        return `instagram_${map[k]||k}_${OV_PID}_${stamp}`;
    }
    ...
    async function runCard(areaId, cardKey, type, btn) {
        ...
        try {
            ...
            console.error('[IGExport.runCard]',err); _toast('Export gagal: '+err.message,'error');
