...
const FEAData={
    ...
    exportCsv(){const posts=filteredPosts.length?filteredPosts:allPosts;if(!posts.length){alert('Tidak ada data.');return;}const hdr='index;nama;emosi;sentimen;likes;comments;tanggal;url;konten';const rows=posts.map((p,i)=>{const name=getName(p),sent={pos:'Positif',neg:'Negatif',neu:'Netral'}[normSent(p)];const content=(p.content||'').replace(/<[^>]*>/g,'').trim().replace(/;/g,',').replace(/\n/g,' ').slice(0,300);return `${i+1};${name.replace(/;/g,',')};${p.emotion||''};${sent};${getLikes(p)};${getComments(p)};${(p.date_created||'').split('T')[0]};${p.url||''};${content}`;});const blob=new Blob(['\uFEFF'+[hdr,...rows].join('\r\n')],{type:'text/csv;charset=utf-8;'});const a=Object.assign(document.createElement('a'),{href:URL.createObjectURL(blob),download:`Instagram_EmotionAnalysis_${FEACfg.sd}_${FEACfg.ed}.csv`});a.click();}
};

/* Charts - identical structure to TikTok */
const FEAChart={
