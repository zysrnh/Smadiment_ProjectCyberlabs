...
<style>
  :root {
    ...
    --red:                  #ff0000;
    --red-dark:             #cc0000;
    --red-light:            rgba(255,0,0,.08);
    --red-border:           rgba(255,0,0,.2);
    --green:                #10b981;
    --green-dark:           #059669;
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ms-stat-card--red   { --stat-bar:linear-gradient(90deg,#ff0000,#cc0000); }
  ...
  .ms-tab-btn.active--red   { background:linear-gradient(135deg,#ff0000 0%,#cc0000 100%)!important;box-shadow:0 4px 12px rgba(255,0,0,.25)!important; }
  ...
  .do-card--red::before   { background:linear-gradient(90deg,#ff0000,#cc0000); }
  ...
  .do-head-icon--red svg { stroke:#ff0000!important; }
  ...
  .fme-post:hover { background:#fff5f5; }
  ...
  .fme-post-av { width:42px;height:42px;border-radius:50%;flex-shrink:0;background:linear-gradient(135deg,#ff0000,#cc0000);color:#fff;font-weight:700;font-size:13px;display:flex;align-items:center;justify-content:center;border:2px solid var(--border-gray);overflow:hidden; }
  ...
  .fme-post-metric--red   { background:var(--red-light);border-color:var(--red-border);color:#ff0000; }
  ...
  .fme-post-metric--red svg   { stroke:#ff0000; }
  ...
  .fme-view-link { display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:700;color:#ff0000;text-decoration:none;padding:3px 9px;border-radius:20px;background:var(--red-light);border:1px solid var(--red-border);transition:var(--transition);margin-left:auto; }
  .fme-view-link:hover { background:#ff0000;color:#fff; }
  ...
  .fme-spinner { width:34px;height:34px;border:3px solid var(--border-gray);border-top-color:#ff0000;border-radius:50%;animation:fmeSpin .7s linear infinite; }
  ...
  .msp-count { background:#ff0000;color:#fff;border-radius:10px;padding:1px 9px;font-size:11px;font-weight:800;flex-shrink:0; }
  ...
  .msp-export-btn { display:flex;align-items:center;gap:5px;padding:5px 11px;background:#ff0000;color:#fff;border:none;border-radius:var(--radius-xs);font-family:var(--font);font-size:10px;font-weight:700;cursor:pointer;transition:var(--transition);white-space:nowrap; }
  .msp-export-btn:hover { background:#cc0000;transform:translateY(-1px); }
  ...
  .msp-item:hover { background:#fff5f5; }
  .msp-avatar { width:38px;height:38px;border-radius:50%;flex-shrink:0;background:linear-gradient(135deg,#ff0000,#cc0000);color:#fff;font-weight:700;font-size:13px;display:flex;align-items:center;justify-content:center;border:1.5px solid var(--border-gray);overflow:hidden; }
  ...
  .msp-spinner { width:32px;height:32px;border:3px solid var(--border-gray);border-top-color:#ff0000;border-radius:50%;animation:msSpin .7s linear infinite; }
  ...
  .msdp-avatar-lg { width:52px;height:52px;border-radius:50%;background:linear-gradient(135deg,#ff0000,#cc0000);color:#fff;font-weight:700;font-size:18px;display:flex;align-items:center;justify-content:center;border:2px solid var(--border-gray);overflow:hidden;flex-shrink:0; }
  ...
  .msdp-yt-embed { position:relative;width:100%;padding-top:56.25%;border-radius:10px;overflow:hidden;margin-bottom:12px;background:#000; }
  .msdp-yt-embed iframe { position:absolute;top:0;left:0;width:100%;height:100%;border:none; }
  /* Thumbnail with play button (before embed loads) */
  .msdp-yt-thumb { position:relative;width:100%;border-radius:10px;overflow:hidden;margin-bottom:12px;background:#000;cursor:pointer; }
  .msdp-yt-thumb img { width:100%;display:block;max-height:200px;object-fit:cover; }
  .msdp-yt-thumb-play { position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.3);transition:background .2s; }
  .msdp-yt-thumb:hover .msdp-yt-thumb-play { background:rgba(0,0,0,.5); }
  .msdp-yt-thumb-play svg { width:56px;height:56px;filter:drop-shadow(0 2px 8px rgba(0,0,0,.5)); }
  ...
  .msdp-link-btn { display:flex;align-items:center;justify-content:center;gap:8px;padding:10px 14px;background:#ff0000;color:#fff;border-radius:10px;font-size:12px;font-weight:700;text-decoration:none;transition:var(--transition);width:100%;margin-top:4px; }
  .msdp-link-btn:hover { background:#cc0000; }
...
<div class="fme-page">
  ...
  <div class="page-header">
    <div class="page-header-left">
      <h1>
        <svg viewBox="0 0 24 24" style="width:28px;height:28px;fill:#ff0000;display:inline-block;vertical-align:middle;margin-right:10px;margin-top:-3px;">
  ...
  <div class="ms-stat-grid">
    <div class="ms-stat-card ms-stat-card--red">
      <div class="ms-stat-label"><span class="ms-stat-dot" style="background:#ff0000;"></span>Total Views</div>
  ...
  <div class="do-card do-card--red ms-mb20" id="donutMasterCard">
    <div class="do-card-head">
      <div class="do-card-head-left">
        <div class="do-head-icon do-head-icon--red" id="donutMasterIco">
    ...
    <div class="do-card-body">
      <div id="donutViewWrap">
        ...
        <div id="donutViewEmpty" style="display:none;"><div class="do-empty"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span class="do-empty-text">Tidak ada data</span></div></div>
      </div>
  ...
  {{-- TABS --}}
  <div class="ms-tabs">
    <button class="ms-tab-btn active active--red" id="tab-view" onclick="FMETab.show('view')">
  ...
  <div class="ms-tab-panel active" id="panel-view">
    <div class="do-card do-card--red ms-mb20">
      <div class="do-card-head">
        <div class="do-card-head-left">
          <div class="do-head-icon do-head-icon--red">
            <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
          </div>
          <div>
            <div class="do-card-title">Top Videos by Views</div>
            <div class="do-card-subtitle">Video YouTube dengan paling banyak views — klik untuk detail</div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;flex-shrink:0;">
          ...
          <span class="do-badge do-badge--red" id="badge-view-full">Loading…</span>
        </div>
      </div>
    ...
    <div class="do-card do-card--ink ms-mb20">
      <div class="do-card-head">
        <div class="do-card-head-left">
          ...
          <div><div class="do-card-title">View vs Like vs Comment — Top 10 Videos</div><div class="do-card-subtitle">Perbandingan engagement keseluruhan pada video terpopuler</div></div>
        </div>
      ...
      <div class="do-card-head">
        <div class="do-card-head-left">
          ...
          <div>
            ...
            <div class="do-card-subtitle">Video YouTube dengan paling banyak likes — klik untuk detail</div>
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;flex-shrink:0;">
    ...
    <div class="do-card do-card--amber ms-mb20">
      <div class="do-card-head">
        <div class="do-card-head-left">
          ...
          <div><div class="do-card-title">Top Videos by Comments</div><div class="do-card-subtitle">Video YouTube dengan paling banyak komentar — klik untuk detail</div></div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;flex-shrink:0;">
...
const FMECfg = {
  ...
  sd:  '{{ $startDate }}',
  ed:  '{{ $endDate }}',
  colors: { view:'#ff0000', like:'#10b981', comment:'#f59e0b' },
  perPage: 10
};
...
/* Fix double-encoded UTF-8 strings from API (Mojibake fix) */
const decodeStr = s => {
  if(!s) return '';
  try {
  ...
  try {
    return s
      ...
      .replace(/Ã¯/g,'ï').replace(/Ã®/g,'î').replace(/Ã´/g,'ô').replace(/Ã»/g,'û')
      .replace(/Ã§/g,'ç').replace(/Ã¹/g,'ù').replace(/Ã¼/g,'ü').replace(/Ã¶/g,'ö')
      .replace(/Ã¤/g,'ä').replace(/Ã¦/g,'æ').replace(/Ã¸/g,'ø').replace(/Ã•/g,'Õ')
...
/* Get YouTube video ID from item - check multiple fields */
const ytVideoIdFromItem = item => {
  const directFields = [item.video_id, item.yt_id, item.youtube_id, item.vid_id, item.sub_id, item.docid];
  ...
  const url = item.url || item.link || item.video_url || item.yt_url || '';
  return ytVideoId(url);
};

/* Get YouTube thumbnail URL */
const ytThumb = (vidOrUrl, quality) => {
  // Accept either a video ID directly or a URL
  let vid = vidOrUrl;
  if(vidOrUrl && (vidOrUrl.includes('youtube') || vidOrUrl.includes('youtu.be') || vidOrUrl.includes('/'))) {
    vid = ytVideoId(vidOrUrl);
  }
  if(!vid) return '';
  return `https://img.youtube.com/vi/${vid}/${quality||'mqdefault'}.jpg`;
};

const ytThumbFromItem = (item, quality) => {
  ...
  if(!vid) return '';
  return `https://img.youtube.com/vi/${vid}/${quality||'hqdefault'}.jpg`;
};
...
/* ══════════════════════════════════════════════════════
   TABS
══════════════════════════════════════════════════════ */
const tabColorMap = { view:'red', like:'green', comment:'amber' };
const tabTitleMap = {
...
const FMETab = {
  ...
  show(type) {
    ['view','like','comment'].forEach(t => {
      ...
      const wrap=document.getElementById('donut'+capitalized+'Wrap');
      const isThis=t===type;
      if(tb) {
        tb.classList.toggle('active',isThis);
        tb.classList.remove('active--red','active--green','active--amber');
    ...
    else { requestAnimationFrame(()=>{ ['ch-view','ch-like','ch-comment','ch-eng'].forEach(id=>{ const c=FMECharts._i[id]; try{if(c&&!c.isDisposed())c.resize();}catch(e){} }); }); }
  },
...
const FMEData = {
  async loadAll() {
    if(!FMECfg.pid) {
      ['valTotalViews','valTotalLikes','valTotalCmts'].forEach(id=>{const e=document.getElementById(id);if(e)e.innerHTML='<span style="font-size:14px;color:#94a3b8;">—</span>';});
      ['list-view','list-like','list-comment'].forEach(id=>{const e=document.getElementById(id);if(e)e.innerHTML=emptyH('Pilih project terlebih dahulu');});
      ['sk-view','sk-like','sk-comment','sk-eng','donutViewSkel','donutLikeSkel','donutCommentSkel'].forEach(hideSk);
      return;
    }
  ...
  async loadTab(type) {
    ...
    try {
      ...
      ['sk-'+type,'donut'+capitalized+'Skel'].forEach(hideSk);
    }
  },
  ...
  _sort(items,type) {
    const keys={
      view:['num_views','view_cnt','views','freq','view_count'],
      like:['num_likes','likes','like_count'],
      comment:['num_comments','comment_count','comments']
    };
    const ks=keys[type]||['num_views'];
  ...
  _updateStats(items) {
    let tV=0,tL=0,tC=0;
    items.forEach(i=>{
      tV+=parseInt(i.num_views||i.view_cnt||i.views||i.freq||0);
      tL+=parseInt(i.num_likes||i.likes||0);
      tC+=parseInt(i.num_comments||i.comments||0);
    });
    ...
    listEl.querySelectorAll('.fme-post').forEach((el)=>{
      el.addEventListener('click',()=>{
        try {
          ...
          const labelMap={view:'Most Viewed Videos',like:'Most Liked Videos',comment:'Most Comments Videos'};
  ...
  _getName(item) {
    const authorName = (item.author_name||'').replace(/<[^>]*>/g,'').trim();
    if(authorName) return authorName;
    const fallbacks = [item.channel_name, item.username, item.name];
    const isChannelId = s => /^UC[A-Za-z0-9_-]{20,}$/.test((s||'').trim());
    for(const f of fallbacks) {
      const v = (f||'').replace(/<[^>]*>/g,'').trim();
      if(v && !isChannelId(v)) return v;
    }
    const scr = (item.author_scr_name||item.author_id||'').replace(/<[^>]*>/g,'').trim();
    if(scr) return scr;
    return 'YouTube Channel';
  },
  _getAvatar(item) {
    return (item.avatar_url||item.profile_url||item.author_image||item.picture||'').trim();
  },
  _getInitials(name) {
    if(!name||name==='YouTube Channel') return 'YT';
    const words=name.replace(/[^a-zA-Z0-9\s]/g,'').split(/\s+/).filter(Boolean);
    if(words.length>=2) return (words[0][0]+words[1][0]).toUpperCase();
    return (words[0]?.[0]||'Y').toUpperCase();
  },
  _getAvatarColor(item) {
    const seed=item.author_id||item.id||this._getName(item)||'yt';
    const colors=['#ff0000','#cc0000','#10b981','#f59e0b','#8b5cf6','#06b6d4','#ec4899','#14b8a6','#f97316','#6366f1'];
  ...
  _postHTML(item,globalIdx,type) {
    ...
    const videoTitle=decodeStr((item.title||item.name||'').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim());
    ...
    // FIX: build ytUrl from vidId if item.url is empty
    const vidId=ytVideoIdFromItem(item);
    const url=item.url||item.link||(vidId?`https://www.youtube.com/watch?v=${vidId}`:'');
    const views=parseInt(item.num_views||item.view_cnt||item.views||item.freq||0);
    ...
    const vV=type==='view'?' fme-post-metric--red':'';
    ...
    return `<div class="fme-post" data-item="${esc(itemEnc)}" data-name="${esc(name)}">
      ...
      <div class="fme-post-body">
        ...
        ${videoTitle?`<div class="fme-post-text" style="font-weight:600;color:var(--text-primary);margin-bottom:4px;">${esc(videoTitle)}</div>`:''}
        ${content&&content!==videoTitle?`<div class="fme-post-text">${esc(content)}</div>`:''}
        <div class="fme-post-stats">
          <span class="fme-post-metric${vV}"><svg viewBox="0 0 24 24" stroke="${type==='view'?'#ff0000':'currentColor'}"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>${numFmt(views)}</span>
  ...
  _renderBar(type,items) {
    ...
    const metricLbl=type==='view'?'Views':type==='like'?'Likes':'Comments';
    chart.setOption({
      animation:true,animationDuration:800,animationEasing:'elasticOut',backgroundColor:'#ffffff',
      tooltip:{...EC_TT,trigger:'axis',axisPointer:{type:'shadow',shadowStyle:{color:'rgba(255,0,0,.04)'}},
        formatter:p=>{
          ...
          const itemVidId=it?ytVideoIdFromItem(it):null;
          const thumb=itemVidId?`https://img.youtube.com/vi/${itemVidId}/mqdefault.jpg`:'';
          const thumbHtml=thumb?`<div style="margin-bottom:8px;border-radius:6px;overflow:hidden;"><img src="${thumb}" style="width:100%;height:100px;object-fit:cover;display:block;" onerror="this.parentElement.style.display='none'"></div>`:'';
  ...
  _renderDonut(type,items) {
    ...
    const activeTab=['view','like','comment'].find(t=>document.getElementById('tab-'+t)?.classList.contains('active'))||'view';
    const masterLeg=document.getElementById('donutMasterLegend');
    ...
    const pieData=top5.map((it,i)=>{
      ...
      const rawContent=(it.content||it.caption||it.title||'').replace(/<[^>]*>/g,'').trim();
      ...
      return {name,value:val,content,sentiment,vidId,color:DONUT_COLORS[i],itemStyle:{color:DONUT_COLORS[i],borderColor:'#fff',borderWidth:3}};
    });
    ...
    chart.setOption({
      ...
      tooltip:{
        ...
        formatter:function(p){
          ...
          const rawContent=decodeStr(d.content||'');
          const snip=rawContent?(rawContent.length>100?rawContent.slice(0,100)+'…':rawContent):'';
          const sc=sC(d.sentiment),sb=sB(d.sentiment);
          const thumb=d.vidId?`https://img.youtube.com/vi/${d.vidId}/mqdefault.jpg`:'';
          const thumbHtml=thumb?`<div style="margin-bottom:10px;border-radius:8px;overflow:hidden;background:#000;"><img src="${thumb}" style="width:100%;height:120px;object-fit:cover;display:block;" onerror="this.parentElement.style.display='none'"></div>`:'';
  ...
  exportCsv(type) {
    ...
    const header='index;nama;sentiment;views;likes;comments;tanggal;url;konten';
    const rows=items.map((it,i)=>{
      const name=this._getName(it);
      const sentRaw=String(it.sentiment_str||it.class_sentiment||it.sentiment||'').toLowerCase();
      const sent=sentRaw.includes('pos')?'Positif':sentRaw.includes('neg')?'Negatif':'Netral';
      const views=parseInt(it.num_views||it.view_cnt||it.views||it.freq||0);
      ...
      return `${i};${name.replace(/;/g,',')};${sent};${views};${likes};${cmts};${dt};${url};${content}`;
    });
    const blob=new Blob(['\uFEFF'+[header,...rows].join('\r\n')],{type:'text/csv;charset=utf-8;'});
    const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`YouTube_Most${type.charAt(0).toUpperCase()+type.slice(1)}_${FMECfg.sd}_${FMECfg.ed}.csv`; a.click();
  }
};
...
const FMEChart = {
  ...
  _render(items, stackType) {
    ...
    const views   =items.map(it=>parseInt(it.num_views||it.view_cnt||it.views||it.freq||0));
    ...
    const labelOption = {
      show:true, position:'insideBottom', distance:15, align:'left',
      verticalAlign:'middle', rotate:90,
      formatter:p=>p.value>0?numK(p.value):'',
      fontSize:9, fontFamily:"'Poppins',sans-serif", fontWeight:'700', color:'#fff'
    };
    const labelOptionStack = {
      show:true, position:'inside',
      fontFamily:"'Poppins',sans-serif", fontWeight:'700', fontSize:9, color:'#fff',
      formatter:p=>p.value>0?numK(p.value):''
    };
    ...
    chart.setOption({
      ...
      tooltip:{
        ...EC_TT,trigger:'axis',axisPointer:{type:'shadow',shadowStyle:{color:'rgba(255,0,0,.04)'}},
        formatter:params=>{
      ...
      legend:{
        bottom:0, data:['Views','Likes','Comments'],
      ...
      yAxis:{type:'value',axisLine:{show:false},axisTick:{show:false},splitLine:{lineStyle:{color:'#f1f5f9',type:'dashed'}},axisLabel:{fontFamily:"'Poppins',sans-serif",fontSize:10,color:'#94a3b8',formatter:numK}},
      series:[
        { name:'Views',   type:'bar',barGap:0,stack:isStack?'eng':undefined,data:views,   barMaxWidth:isStack?54:32,itemStyle:{color:'#ff0000',borderRadius:isStack?[0,0,0,0]:[4,4,0,0]},emphasis:{focus:'series'},label:isStack?labelOptionStack:labelOption },
        { name:'Likes',   type:'bar',barGap:0,stack:isStack?'eng':undefined,data:likes,   barMaxWidth:isStack?54:32,itemStyle:{color:'#10b981',borderRadius:isStack?[0,0,0,0]:[4,4,0,0]},emphasis:{focus:'series'},label:isStack?labelOptionStack:labelOption },
        { name:'Comments',type:'bar',barGap:0,stack:isStack?'eng':undefined,data:cmts,    barMaxWidth:isStack?54:32,itemStyle:{color:'#f59e0b',borderRadius:isStack?[4,4,0,0]:[4,4,0,0]},emphasis:{focus:'series'},label:isStack?labelOptionStack:labelOption }
      ]
    },true);
    chart.on('click',params=>{
      const item=items[params.dataIndex]; if(!item) return;
      const seriesNameMap={'Views':'view','Likes':'like','Comments':'comment'};
      const clickedType=seriesNameMap[params.seriesName]||'view';
...
const FMEPopup = {
  ...
  open(items,type,title,count) {
    const popup=document.getElementById('msPopup');
    const colorMap={view:'#ff0000',like:'#10b981',comment:'#f59e0b'};
    this._items=items||[]; this._curType=type;
    FMEDetail.close();
    document.getElementById('msPopDot').style.background=colorMap[type]||'#ff0000';
  ...
  _render(list,items,type) {
    ...
    list.innerHTML=items.slice(0,100).map((item)=>{
      ...
      // FIX: include num_views in views fallback
      const views=parseInt(item.num_views||item.view_cnt||item.views||item.freq||0);
      const likes=parseInt(item.num_likes||item.likes||0);
      const cmts=parseInt(item.num_comments||item.comments||0);
      const dt=(item.date_created||'').split('T')[0];
      ...
      const eng=type==='view'?`Views ${numFmt(views)}`:type==='like'?`Like ${numFmt(likes)}`:`Komen ${numFmt(cmts)}`;
      const itemEnc=encodeURIComponent(JSON.stringify(item));
...
const FMEDetail = {
  open(item,type) {
    ...
    const color=colorMap[type]||'#ff0000';
    ...
    const videoTitle=rawTitle?decodeStr(rawTitle):'';
    const isChannelId = s => /^UC[A-Za-z0-9_-]{20,}$/.test((s||'').trim());
    const channelId=(item.author_scr_name||'').trim();
    const channelIdSubtitle = channelId && isChannelId(channelId) ? channelId : '';
    ...
    // FIX: vidId first, then build url from vidId if item.url is empty
    const vidId=ytVideoIdFromItem(item);
    const url=item.url||item.link||(vidId?`https://www.youtube.com/watch?v=${vidId}`:'');
    const thumb=vidId?`https://img.youtube.com/vi/${vidId}/hqdefault.jpg`:(item.thumbnail||item.image_url||item.picture||'').trim();

    let mediaHtml='';
    if(vidId) {
      mediaHtml=`<div class="msdp-yt-thumb" id="msdpThumbWrap" onclick="FMEDetail._loadEmbed('${vidId}')">
        <img src="${thumb}" onerror="this.src='https://img.youtube.com/vi/${vidId}/hqdefault.jpg'" alt="thumbnail">
        <div class="msdp-yt-thumb-play">
      ...
      mediaHtml=`<div class="msdp-media-wrap"><img class="msdp-media-img" src="${esc(thumb)}" onerror="this.parentElement.style.display='none'"></div>`;
    }
    ...
    body.innerHTML=`
      <div class="msdp-avatar-row">
        ...
        <div style="min-width:0;">
          <div class="msdp-author-name" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(name)}</div>
          ${channelIdSubtitle?`<div style="font-size:10px;color:#94a3b8;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${esc(channelIdSubtitle)}</div>`:''}
          <span style="background:${avColor}22;color:${avColor};padding:2px 10px;border-radius:20px;font-size:10px;font-weight:700;display:inline-block;margin-top:4px;">YouTube · ${label}</span>
        </div>
      </div>
      ${dtFmt?`<div class="msdp-meta-row"><span>${dtFmt}</span></div>`:''}
      <span class="msdp-sent-badge msdp-sent-badge--${sent}">${sentLbl}</span>
      ${mediaHtml}
      ${videoTitle?`<div style="font-size:14px;font-weight:700;color:var(--text-primary);margin-bottom:10px;line-height:1.5;">${esc(videoTitle)}</div>`:''}
      ${content && content !== videoTitle ? `<div class="msdp-content-text">${esc(content)}</div>` : ''}
      <div class="msdp-stats-grid">
        <div class="msdp-stat-box"><div class="msdp-stat-val">${numFmt(views)}</div><div class="msdp-stat-lbl">Views</div></div>
  ...
  _loadEmbed(vidId) {
    const wrap=document.getElementById('msdpThumbWrap');
    if(!wrap) return;
    const div=document.createElement('div');
    div.className='msdp-yt-embed';
...
const FME = {
  reload() {
    ...
    FMETab.reset();
    Store.view=[]; Store.like=[]; Store.comment=[];
    FMEChart._items=[];
    FMEData.loadAll();
  },
  init() {
