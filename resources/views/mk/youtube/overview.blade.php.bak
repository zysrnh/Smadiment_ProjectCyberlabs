...
<style>
  :root {
    ...
    --yt-color: #FF0000;
    --yt-color-dark: #cc0000;
    --yt-color-light: #ff4444;
  }
  ...
  .stat-card:hover { transform: translateY(-4px); box-shadow: var(--shadow-lg); border-color: var(--yt-color); }
  ...
  .stat-icon { width: 28px; height: 28px; stroke: #ffffff; fill: none; stroke-width: 2; }
  ...
  @media (min-width: 1024px) {
    .charts-section { grid-template-columns: 1.2fr 0.8fr; }
  }
  ...
  .account-name-link:hover { color: var(--yt-color); text-decoration: underline; }
  ...
  .view-all-btn:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(255,0,0,0.3); }
  ...
  .all-users-modal .modal-overlay { position: absolute; top: 0; left: 0; right: 0; bottom: 0; }
  ...
  .all-users-modal .modal-header h3 { font-size: 18px; font-weight: 700; color: var(--text-primary); margin: 0; }
  .all-users-modal .modal-body { padding: 0; overflow-y: auto; border-radius: 0 0 16px 16px; }
  ...
  [data-lazy-load] { opacity: 0; transform: translateY(20px); transition: opacity 0.6s ease-out, transform 0.6s ease-out; }
  ...
  .date-preset:hover { background: var(--bg-white); color: var(--yt-color); }
  ...
  .calendar-day.today       { border: 2px solid var(--yt-color); }
  .calendar-day.selected    { background: var(--yt-gradient); color: white; }
  .calendar-day.in-range    { background: rgba(255,0,0,0.1); color: var(--yt-color); }
  .calendar-day.range-start,
  .calendar-day.range-end   { background: var(--yt-gradient); color: white; }
  ...
  .apply-date-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(255,0,0,0.35); }
...
<div class="dashboard-container">
    ...
    <h1>
      <div class="yt-icon-wrapper">
        <svg viewBox="0 0 24 24" fill="white" stroke="none">
          ...
          <polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02" fill="red"/>
        </svg>
      </div>
      YouTube Overview Dashboard
    </h1>
    <p>Monitor and analyze your YouTube channel performance and engagement metrics</p>
  </div>
  ...
  <div class="alert alert-warning">
    ...
    <span>No project selected. Please select a project from the sidebar to view YouTube Overview data.</span>
  </div>
  @else

  <!-- Date Filter Card -->
  <div class="filter-card">
    <form id="filterForm" method="GET" action="{{ route('mk.youtube.overview') }}">
    ...
    <div class="chart-card" data-lazy-load="volumeTotal">
      <div class="chart-header">
        <div class="chart-title-group">
          <h3>Volume Trend</h3>
          <p class="chart-subtitle">Daily video upload volume over time</p>
        </div>
      </div>
...
if (projectId && startDate && endDate) {
  ...
  async function loadTotalUsers() {
    ...
    try {
      const res    = await fetch(`/mk/api/youtube/total-users?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
  ...
  async function loadTotalAuthors() {
    ...
    addLoadingBadge(card);
    try {
      const res    = await fetch(`/mk/api/youtube/total-authors?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
      const result = await res.json();
      if (result.success) {
  ...
  async function loadVolumeTotal() {
    ...
    try {
      const res    = await fetch(`/mk/api/youtube/volume-total?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
      const result = await res.json();
      if (result.success) {
      ...
      console.error(e);
      const el = document.getElementById('volumeTotalValue');
      if (el) el.innerHTML = `<div class="stat-value">0</div>`;
  ...
  async function loadSentimentTotal() {
    ...
    cards.forEach(c => addLoadingBadge(c));
    try {
      const res    = await fetch(`/mk/api/youtube/sentiment-total?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
      const result = await res.json();
      if (result.success) {
  ...
  async function loadMostActiveUsers() {
    ...
    addLoadingBadge(card);
    try {
      const res    = await fetch(`/mk/api/youtube/most-active-users?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
      const result = await res.json();
      ...
      if (result.success && result.data?.data && result.data.data.length > 0) {
        ...
        container.innerHTML = `
          <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 20px;gap:16px;">
            <svg viewBox="0 0 24 24" style="width:56px;height:56px;stroke:#cbd5e1;fill:none;stroke-width:1.5;">
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/>
              <circle cx="9" cy="7" r="4"/>
              <path d="M23 21v-2a4 4 0 0 0-3-3.87"/>
              <path d="M16 3.13a4 4 0 0 1 0 7.75"/>
            </svg>
            <p style="color:#94a3b8;font-size:15px;font-weight:600;margin:0;">No active channels found</p>
            <p style="color:#cbd5e1;font-size:13px;margin:0;">No YouTube channel activity detected for this period</p>
          </div>`;
      }
    } catch (e) {
      console.error(e);
      loading.style.display   = 'none';
      container.style.display = 'block';
      container.innerHTML = `<div style="text-align:center;padding:40px;color:#94a3b8;">Failed to load channel data</div>`;
    }
    finally { removeLoadingBadge(card); card.classList.add('loaded'); }
  }
  ...
  function buildTableHTML(users) {
    let html = `<table class="data-table"><thead><tr>
      <th>NO.</th><th>AVATAR</th><th>CHANNEL</th><th>VIDEOS</th><th>LIKES</th><th>COMMENTS</th>
    </tr></thead><tbody>`;
    ...
    users.forEach((item, i) => {
      ...
      html += `<tr>
        ...
        <td><a href="https://youtube.com/@${name}" target="_blank" class="account-name-link">${name}</a></td>
  ...
  function filterUsers() {
    ...
    container.innerHTML = (term && filtered.length === 0)
      ? `<table class="data-table"><tbody><tr><td colspan="6" style="text-align:center;padding:40px;color:var(--text-secondary);">No channels found matching "${term}"</td></tr></tbody></table>`
    ...
    modal.innerHTML = `
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h3>All Active Channels (${allUsers.length} total)</h3>
  ...
  function renderVolumeTrendChart(data) {
    ...
    const hasData = data && data.length > 0 && data.some(d => (d.count || d.value || 0) > 0);
    ...
    if (!hasData) {
      ...
      loading.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:12px;">
          <svg viewBox="0 0 24 24" style="width:48px;height:48px;stroke:#cbd5e1;fill:none;stroke-width:1.5;">
            <path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z"/>
            <polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02"/>
          </svg>
          <p style="color:#94a3b8;font-size:14px;font-weight:500;margin:0;">No volume data available for this period</p>
          <p style="color:#cbd5e1;font-size:12px;margin:0;">Try selecting a different date range</p>
        </div>`;
      return;
    }
    ...
    new Chart(canvas.getContext('2d'), {
      ...
      options: {
        ...
        scales: {
          y: { beginAtZero: true, grid: { color: '#f1f5f9', drawBorder: false }, ticks: { color: '#64748b', font: { family: 'Poppins', size: 12 }, padding: 8 } },
          x: { grid: { display: false, drawBorder: false }, ticks: { color: '#64748b', font: { family: 'Poppins', size: 12 }, padding: 8 } }
        }
      }
    });
  ...
  function renderSentimentChart(sentiment) {
    ...
    if (total === 0) {
      ...
      loading.innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:12px;">
          ...
          <p style="color:#94a3b8;font-size:14px;font-weight:500;margin:0;">No sentiment data available</p>
        </div>`;
      return;
    }
    ...
    new Chart(canvas.getContext('2d'), {
      ...
      options: {
        ...
        cutout: '70%',
        plugins: {
          legend: { position: 'bottom', labels: { color: '#1a202c', font: { family: 'Poppins', size: 13, weight: '600' }, padding: 20, usePointStyle: true, pointStyle: 'circle' } },
          tooltip: {
            backgroundColor: '#1a202c', padding: 16,
            titleColor: '#fff', bodyColor: '#fff',
