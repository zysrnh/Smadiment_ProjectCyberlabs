...
<style>
    ...
    .prompt-item:hover { background: rgba(255,0,0,0.06); color: var(--primary); }
    .prompt-item.active {
        background: rgba(255,0,0,0.1); color: var(--primary);
    ...
    .active-prompt-bar {
        padding: 7px 18px; background: rgba(255,0,0,0.05);
    ...
    .btn-send {
        ...
        box-shadow: 0 3px 10px rgba(255,0,0,0.32); transition: all 0.15s; flex-shrink: 0;
    }
    .btn-send:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 5px 16px rgba(255,0,0,0.42); }
    ...
    .calendar-day.in-range { background: rgba(255,0,0,0.1); color: var(--primary); }
    .dp-display {
            ...
            <div class="welcome-state" id="welcomeState">
                <div class="welcome-icon-wrap">
                    <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="30" height="30">
                        <path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z" fill="rgba(255,255,255,0.3)"/>
                        <polygon points="9.75,8.98 9.75,15.02 15.5,12" fill="white"/>
                    </svg>
                </div>
                <h3>Ready to Analyze YouTube</h3>
                <p>Pilih template dari panel kiri, atau ketik pertanyaan sendiri untuk menganalisis data percakapan YouTube pada project ini.</p>
...
async function preloadProjectData() {
    ...
    try {
        ...
        setReady(`${s.total_videos} videos · +${Math.round(pos/tot*100)}% / -${Math.round(neg/tot*100)}% · ${s.total_hashtags} hashtags`);
    } catch (err) {
        cachedDataset = `Project ID: ${PROJECT_ID}, Platform: YouTube, Periode: ${START_DATE} s/d ${END_DATE}`;
        dataReady = true;
...
function isAnalyticalMessage(text) {
    ...
    const kw = ['analisis','analysis','youtube','ytb','video','channel','shorts','view','subscriber','comment','like','sentimen','sentiment','isu','issue','topik','topic','kreator','creator','konten','content','viral','trending','mention','thumbnail','hashtag','swot','pestle','scct','audience','narasi','narrative','krisis','crisis','komunikasi','communication','engagement','rangkum','ringkas','summarize','tren','trend','pola','siapa','apa','bagaimana','mengapa','data','laporan','report','butterfly','framing','entman','edelman','wacana','fairclough','vandijk','wodak','intelijen','mcdowell','prunckun','sherman','kent','hybrid','warfare','agenda','counter','riding','parpol'];
...
async function sendMessage() {
    ...
    const typingEl = appendTyping('Menganalisis data YouTube…');
...
function appendMsg(role, text) {
    ...
    const ava = `width:32px;height:32px;min-width:32px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0;color:#fff !important;background:${isAI?'linear-gradient(135deg,#038047 0%,#026738 100%)':'linear-gradient(135deg,#FF0000 0%,#cc0000 100%)'} !important;box-shadow:${isAI?'0 2px 8px rgba(3,128,71,.25)':'0 2px 8px rgba(255,0,0,.25)'};`;
    const bub = isAI
...
function clearChat() {
    ...
    chatHistory=[];
    document.getElementById('aiMessages').innerHTML=`<div class="welcome-state" id="welcomeState"><div class="welcome-icon-wrap"><svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" width="30" height="30"><path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.95A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z" fill="rgba(255,255,255,0.3)"/><polygon points="9.75,8.98 9.75,15.02 15.5,12" fill="white"/></svg></div><h3>Ready to Analyze YouTube</h3><p>Pilih template dari panel kiri atau ketik pertanyaan sendiri.</p></div>`;
    clearActivePrompt();
}
