...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
    ...
    <div class="d-flex align-items-center gap-2">
        <small class="text-muted" style="font-size:11px;font-weight:600;">Mode:</small>
...
/* ── Stopwords (expanded for Indonesian YouTube) ── */
...
function getSent(name) {
    const tk = name.toLowerCase().replace(/^#/,'').trim().split(/[^a-z0-9]+/).filter(Boolean);
...
function extractKeywords(posts) {
    const c = {};
    posts.forEach(p => {
        /* YouTube titles are the richest signal */
...
/* ══════════════════════════════════════════════════════
   LOAD DATA — 4-step fallback chain

   Step 1 — PRELOADED_RAW  (server pre-loaded, fastest)
   Step 2 — /trending-topics API  (controller extraction)
   Step 3 — /most-engagement posts → extract client-side
   Step 4 — Empty state
══════════════════════════════════════════════════════ */
async function loadData() {
    ...
    if (Array.isArray(PRELOADED_RAW) && PRELOADED_RAW.length >= 3) {
        const data = PRELOADED_RAW
            .map(t => ({ name:(t.hashtag||t.name||'').trim(), size:+(t.size||t.total_volume||t.appearances||1) }))
    ...
    /* ── Step 2: trending-topics API (server does extraction) ── */
    try {
    ...
    try {
        ...
        if (posts.length) {
            ...
            if (hashtags.length >= 5) {
                ...
                /* Merge: hashtags boosted × 3 + keywords */
                const merged = [
...
function buildTopics(data, mode) {
    ...
    allTopics = data.map(t => ({name:t.name, size:t.size, sent:getSent(t.name)})).filter(t=>t.name);
...
function updateKpi() {
    ...
    const pos=allTopics.filter(t=>t.sent==='positive').length,neg=allTopics.filter(t=>t.sent==='negative').length;
    const el=(id,v)=>{const e=_$(id);if(e)e.textContent=numF(v);};
    el('kpiTopics',n); _$('kpiTopicsSub').innerHTML=`<i class="ph ph-hash me-1"></i>${n} trending topics`;
...
function showEmpty() {
    ['wcLoading','wcEmpty','topicLoading','topicEmpty'].forEach(id=>{ const e=_$(id); if(e) e.style.display=id.includes('Loading')?'none':'flex'; });
    ['kpiTopics','kpiVolume','kpiPos','kpiNeg'].forEach(id=>{ const e=_$(id); if(e) e.textContent='0'; });
}

/* ══════════════════════════════════════════════════════
   WORD CLOUD RENDER
   KEY FIX: requestAnimationFrame + getBoundingClientRect
   guarantees ECharts gets real pixel dimensions
══════════════════════════════════════════════════════ */
function renderWC() {
    ...
    requestAnimationFrame(()=>{
        ...
        if(rect.width===0||rect.height===0){
            /* Container not painted yet — keep spinner, retry */
        ...
        wcChart.setOption({
            ...
            series:[{
                ...
                textStyle:{
                    fontFamily:'inherit',
                    fontWeight:'600',
                    color:()=>['#16a34a','#2563eb','#f59e0b','#ef4444','#9333ea','#14b8a6','#0ea5e9','#d97706'][Math.floor(Math.random()*8)]
                },
...
document.addEventListener('DOMContentLoaded',()=>{
    loadData();
    window.addEventListener('resize',()=>{if(!wcChart)return;const ch=_$('wordCloudChart');if(!ch)return;const r=ch.getBoundingClientRect();try{if(r.width>0&&r.height>0)wcChart.resize({width:r.width,height:r.height});else wcChart.resize();}catch(e){}});
    document.querySelectorAll('.sent-tab').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.sent-tab').forEach(b=>b.classList.remove('active'));btn.classList.add('active');curSent=btn.dataset.s;applyFilter();}));
    document.querySelectorAll('.mode-btn').forEach(btn=>btn.addEventListener('click',()=>{document.querySelectorAll('.mode-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');switchMode(btn.dataset.mode);}));
});
</script>
@endsection