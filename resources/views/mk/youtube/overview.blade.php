...
.tme-post-thumb{width:80px;height:60px;border-radius:var(--radius-sm);flex-shrink:0;overflow:hidden;border:1.5px solid var(--slate-200);background:var(--slate-800);position:relative;align-self:center;box-shadow:var(--shadow-sm)}
...
.do-dp2-media iframe{width:100%;height:280px;border:none;display:block}
...
<div id="pageExportArea">
    ...
    <div class="row mb-3">
        @php
            $kpiCards = [
                ...
                ['id' => 'kpiLikes',    'label' => 'Total Likes',      'icon' => 'ph-thumbs-up',        'bg' => '#F59E0B', 'sub' => 'kpiLikesSub'],
                ['id' => 'kpiComments', 'label' => 'Total Comments',   'icon' => 'ph-chat-circle-dots', 'bg' => '#4CAF50', 'sub' => 'kpiCommentsSub'],
                ['id' => 'kpiEng',      'label' => 'Total Engagement', 'icon' => 'ph-chart-bar',          'bg' => '#038047', 'sub' => 'kpiEngSub'],
            ];
            $delays = ['d1','d2','d3','d4'];
        @endphp
    ...
    <div class="page-export-bar" data-html2canvas-ignore="true">
        ...
        <div class="page-export-bar-right">
            <button type="button" class="page-export-btn page-export-btn-pdf" id="pageExportPdfBtn"
                    onclick="YTExport.run('pdf',this)" title="Export PDF">
            ...
            <button type="button" class="page-export-btn page-export-btn-img" id="pageExportImgBtn"
                    onclick="YTExport.run('image',this)" title="Export PNG">
    ...
    <div class="tme-tabs" id="ovTabsBar">
        <button class="tme-tab-btn active" id="tab-hashtag" onclick="OVTab.show('hashtag')">
            <i class="ph ph-hash"></i> Top Hashtags
        ...
        <button class="tme-tab-btn" id="tab-like" onclick="OVTab.show('like')">
            <i class="ph ph-thumbs-up"></i> Most Liked
            <span class="tme-tab-chip" id="chip-like">—</span>
        </button>
        <button class="tme-tab-btn" id="tab-comment" onclick="OVTab.show('comment')">
            <i class="ph ph-chat-circle"></i> Most Comments
            <span class="tme-tab-chip" id="chip-comment">—</span>
        </button>
    </div>
            ...
            <div id="card-export-hashtag-donut">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <div class="d-flex gap-1" data-html2canvas-ignore="true">
                        <button class="card-exp-btn card-exp-btn-pdf" onclick="YTExport.runCard('card-export-hashtag-donut','hashtag-donut','pdf',this)" title="PDF"><i class="ph ph-file-pdf export-icon"></i><span class="export-spinner"></span></button>
                        <button class="card-exp-btn card-exp-btn-img" onclick="YTExport.runCard('card-export-hashtag-donut','hashtag-donut','image',this)" title="PNG"><i class="ph ph-image export-icon"></i><span class="export-spinner"></span></button>
                    </div>
                </div>
    ...
    {{-- ══ Panel: Most Viewed / Most Liked / Most Comments ══ --}}
    @foreach(['view' => 'Most Viewed', 'like' => 'Most Liked', 'comment' => 'Most Comments'] as $tp => $tpLabel)
    @php
        $tpIcons = ['view' => 'ph-eye', 'like' => 'ph-thumbs-up', 'comment' => 'ph-chat-circle'];
        $tpIcon  = $tpIcons[$tp];
    @endphp
        ...
        <div class="card mb-3" style="animation:fadeUp .38s ease-out .18s both;">
            <div id="card-export-donut-{{ $tp }}">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        <div class="avtar avtar-xs bg-light-primary rounded"><i class="ph ph-chart-donut f-18 text-primary"></i></div>
                        <div><h6 class="mb-0">Distribusi {{ $tpLabel }} — Top 5</h6><small class="text-muted">Proporsi engagement per video — klik untuk detail</small></div>
                    </div>
                    <div class="d-flex gap-1" data-html2canvas-ignore="true">
                        <button class="card-exp-btn card-exp-btn-pdf" onclick="YTExport.runCard('card-export-donut-{{ $tp }}','donut-{{ $tp }}','pdf',this)" title="PDF"><i class="ph ph-file-pdf export-icon"></i><span class="export-spinner"></span></button>
                        <button class="card-exp-btn card-exp-btn-img" onclick="YTExport.runCard('card-export-donut-{{ $tp }}','donut-{{ $tp }}','image',this)" title="PNG"><i class="ph ph-image export-icon"></i><span class="export-spinner"></span></button>
                    </div>
                </div>
                <div class="card-body pb-0">
                    <div class="donut-stack-wrap">
            ...
            <div id="card-export-list-{{ $tp }}">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div><h6 class="mb-0">Top Videos by {{ $tpLabel }}</h6><small class="text-muted">Klik video untuk lihat detail</small></div>
                    </div>
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div class="d-flex gap-1" data-html2canvas-ignore="true">
                            <button class="card-exp-btn card-exp-btn-pdf" onclick="YTExport.runCard('card-export-list-{{ $tp }}','list-{{ $tp }}','pdf',this)" title="PDF"><i class="ph ph-file-pdf export-icon"></i><span class="export-spinner"></span></button>
                            <button class="card-exp-btn card-exp-btn-img" onclick="YTExport.runCard('card-export-list-{{ $tp }}','list-{{ $tp }}','image',this)" title="PNG"><i class="ph ph-image export-icon"></i><span class="export-spinner"></span></button>
                        </div>
                    </div>
                </div>
...
const OVData = {
    ...
    async loadHashtags() {
        ...
        try {
            const r = await fetch(`${API_BASE}/trending-topics?project_id=${OVCfg.pid}&start_date=${OVCfg.sd}&end_date=${OVCfg.ed}`);
            ...
            if (j.success && j.data?.hashtags?.length) {
                ...
                this._renderHashtagList(j.data.hashtags);
    ...
    _renderHashtagList(hashtags) {
        ...
        if(pg) {
            if(pages<=1) { pg.innerHTML=''; }
                ...
                pg.innerHTML=`<div class="tme-pagination"><span class="tme-pag-info">Menampilkan ${fr}–${to} dari ${total} hashtags</span><div class="tme-pag-controls">${b}</div></div>`;
            }
        }
        if(ld) ld.style.display='none';
    ...
    _renderDonutHashtag(h) {
        ...
        this._renderDonutEcharts(
            ...
            'Hashtag', null, '__ec_donutHashtagChart'
        );
    ...
    async loadEngagement(type) {
        ...
        try {
            ...
            if(type==='view') this._updateKpi(items);
    ...
    _getColor(item) {
        const seed=item.video_id||item.docid||item.id||this._getName(item)||'yt';
    ...
    _pagHtml(type,page,pages,total,from,to) {
        ...
        return `<div class="tme-pagination"><span class="tme-pag-info">Menampilkan ${from}–${to} dari ${total} videos</span><div class="tme-pag-controls">${b}</div></div>`;
    },
    ...
    _postHtml(item,gi,type) {
        ...
        const vC=type==='view'?' tme-metric--primary':'',
              lC=type==='like'?' tme-metric--primary':'',
              cC=type==='comment'?' tme-metric--amber':'';
        const th=this._getThumb(item);
        const thH=(th&&th.startsWith('http'))
            ?`<div class="tme-post-thumb"><img src="${esc(th)}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'tme-post-thumb-ph\\'>🎬</div>'"><div class="tme-post-thumb-play"><i class="ph ph-play-fill"></i></div></div>`
    ...
    _renderDonut(type, items) {
        const ld=_$('loadingDonut-'+type), ch=_$('donutChart-'+type),
              em=_$('donutEmpty-'+type), bd=_$('donutBreakdown-'+type);
        if (!ld||!ch) return;
        if (!items.length) { ld.style.display='none'; if(em) em.style.display='flex'; return; }
        const top5=items.slice(0,5), ml={view:'Views',like:'Likes',comment:'Comments'}[type];
        ...
        this._renderDonutEcharts(
            ...
            ml, null, '__ec_donutChart-'+type
        );
        this._renderBreakdownList(bd, top5, total,
            it=>this._getName(it),
            it=>this._metric(it,type),
            it=>{ OVPanel.open(Store[type],type,PLAT); OVDetail.open(it,type); }
        );
    ...
    /* ── Breakdown list di bawah donut (sama persis dengan X) ── */
    _renderBreakdownList(el, data, total, getNameFn, getValFn, onClickFn) {
        ...
        el.innerHTML=data.map((d,i)=>{
            ...
            return `<div class="donut-breakdown-item" data-idx="${i}">
                ...
                <div class="donut-breakdown-bar-wrap"><div class="donut-breakdown-bar-fill" style="width:${barW}%;background:${color};"></div></div>
    ...
    /* ── ECharts donut (sama persis dengan X: label ringkas, breakdown list terpisah) ── */
...
const chart=echarts.init(chartEl, null, {renderer:'canvas'});
      window[ek]=chart;
        ...
        chart.on('mouseover', p => {
            ...
            const d=data[p.dataIndex], cl=DONUT_COLORS[p.dataIndex];
            clearTimeout(_tm);
    ...
    _openHashtagPanel(h) {
        ...
        const filtered=allPostsRaw.filter(p=>(p.content||p.title||p.caption||'').toLowerCase().includes(tag));
...
const OVPanel = {
    ...
    open(items, type, title) {
        ...
        _$('ovPanelTitle').textContent=title||PLAT+' Videos';
    ...
    _render(items,type) {
        ...
        const ml={view:'Views',like:'Likes',comment:'Komentar'}[type]||'Views';
        list.innerHTML=items.slice(0,100).map(item=>{
            const nm=OVData._getName(item), av=OVData._getAvatar(item), cl=OVData._getColor(item),
                  ...
                  tx=(item.content||item.title||'').replace(/<[^>]*>/g,'').trim(),
                  mv=OVData._metric(item,type), dt=(item.date_created||'').split('T')[0],
                  sn=OVData._normSent(item), sl={pos:'Pos',neg:'Neg',neu:'Neu'}[sn],
                  v=parseInt(item.num_views||item.view_cnt||0), l=parseInt(item.num_likes||item.likes||0),
                  c=parseInt(item.num_comments||item.comments||0), total=v+l+c,
            ...
            return `<div class="do-panel-item" data-item="${esc(enc)}" data-type="${type}" onclick="OVPanel._click(this)">
                ...
                <div class="do-panel-item-body">
                    ...
                    <div class="do-panel-text">${esc(dec(tx).slice(0,130)||'(tidak ada konten)')}</div>
                    <div class="do-panel-footer">
                        <span class="do-sent-badge do-sent-badge--${sn}">${sl}</span>
                        <span>${ml} ${numF(mv)}</span><span>∑ ${numF(total)}</span>
...
const OVDetail = {
    open(item, type) {
        ...
        const color=OVCfg.colors[type]||OVCfg.primary, name=OVData._getName(item),
        ...
        let vid=item.video_id||item.docid||'';
        if(!vid&&url){ const m=url.match(/(?:v=|\/embed\/|youtu\.be\/)([a-zA-Z0-9_-]{11})/); if(m) vid=m[1]; }
        const mH=vid?`<div class="do-dp2-media"><iframe src="https://www.youtube.com/embed/${esc(vid)}" allow="autoplay;encrypted-media" allowfullscreen></iframe></div>`:'';
        title.textContent=name;
        body.innerHTML=`<div class="do-dp2-avatar-row">
            ...
            <div>
                ...
                <span class="do-dp2-plat-badge" style="background:${color}18;color:${color};">YouTube</span>
            </div>
        </div>
...
/* ════════════════════════════════════════════════════════
   YTExport — sama persis dengan OVExport di X Overview
   (dengan _snapshotForTab + skipSnapshot untuk PDF multi-tab)
════════════════════════════════════════════════════════ */
const YTExport = (() => {
    'use strict';
    ...
    function _svgToPng(container) {
        return new Promise(resolve => {
            ...
            const w = Math.round(rect.width)  || container.offsetWidth  || 420;
            ...
            image.onerror = () => { clearTimeout(timer); cleanup(); resolve(null); };
            image.src = blobUrl;
        });
    }

    /* ECharts container IDs per tab */
    const TAB_EC_IDS = {
        hashtag : ['donutHashtagChart'],
        view    : ['donutChart-view'],
        like    : ['donutChart-like'],
        comment : ['donutChart-comment'],
    };

    /* ── Swap ECharts SVG → <img> paralel ── */
    async function _swapChartsIn(el, tabKey) {
        const swaps  = [];
        const ids    = tabKey
            ? (TAB_EC_IDS[tabKey] || [])
            : Object.values(TAB_EC_IDS).flat();
        ...
        const results = await Promise.all(ids.map(async id => {
            ...
            try {
                const result = await _svgToPng(container);
                if (!result) return null;
                return { container, imgURI: result.dataUrl, h: result.h };
            } catch(e) {
                console.warn('[YTExport] echarts swap failed:', id, e);
                return null;
            }
        }));
        ...
        results.forEach(r => {
            if (!r) return;
            const { container, imgURI, h } = r;

            const placeholder = document.createElement('div');
            placeholder.dataset.swapFor = container.id;
    ...
    function _onClone(clonedDoc) {
        clonedDoc.querySelectorAll(
            ...
            '.spin-ring,.spinner-state,.export-toast,.chart-loading,[data-html2canvas-ignore]'
        ).forEach(el => {
        ...
        clonedDoc.querySelectorAll('.tme-post-av,.do-panel-avatar,.do-dp2-avatar-lg').forEach(wrapper => {
            ...
            if(!wrapper.querySelector('.__ini')){
                ...
                sp.textContent='Y'; sp.style.cssText='font-size:13px;font-weight:700;color:#fff;line-height:1;';
                wrapper.appendChild(sp);
            }
            if(!wrapper.style.background) wrapper.style.background='linear-gradient(135deg,#FF0000,#cc0000)';
        });
        ...
        clonedDoc.querySelectorAll('*').forEach(el => {
            el.style.animationPlayState='paused'; el.style.animation='none'; el.style.transition='none';
        });
    ...
    /* ── Core capture ── */
    async function _doCapture(el, isCard, tabKey=null) {
    el.querySelectorAll('.card,.kpi-card-hover,[class*="col-"],.tme-post,.ht-item')
      .forEach(e => { e.style.opacity='1'; e.style.transform='none'; e.style.visibility='visible'; });

    // Matikan animasi ECharts dulu sebelum swap
    const ecKeys = ['__ec_donutHashtagChart','__ec_donutChart-view','__ec_donutChart-like','__ec_donutChart-comment'];
    ecKeys.forEach(k => { try { if(window[k]) window[k].setOption({animation:false}); } catch(e){} });

    // Tunggu chart selesai render
    await new Promise(r => setTimeout(r, 600));

    const swaps = await _swapChartsIn(el, tabKey);
    ...
    try {
        canvas = await html2canvas(el, {
            ...
            ignoreElements : e => e.hasAttribute('data-html2canvas-ignore') ||
                                  (e.tagName === 'IMG' && !e.src?.startsWith('data:') && !e.closest('[data-swap-for]')),
        });
    } finally {
        _swapChartsOut(swaps);
        // Kembalikan animasi
        ecKeys.forEach(k => { try { if(window[k]) window[k].setOption({animation:true}); } catch(e){} });
    }
    return canvas;
}
    /* ── PDF helpers ── */
    function _drawHeader(pdf, pW, pH, label, page, total) {
        pdf.setFillColor(255,0,0); pdf.rect(0,0,pW,11,'F');
        pdf.setTextColor(255,255,255); pdf.setFontSize(9); pdf.setFont('helvetica','bold');
        pdf.text('SMADIMENT — '+(label||'YouTube Overview'), 10, 7.5);
    ...
    function _sliceCanvas(pdf, canvas, margin, pW, pH, label, pageStart, totalPages) {
        const uw=pW-margin*2, uh=pH-14-10;
        const ratio=uw/canvas.width, sliceH=uh/ratio;
        let srcY=0, pg=pageStart;
        while(srcY<canvas.height){
            if(pg>pageStart) pdf.addPage();
    ...
    /* ── run: export full page (semua 4 tab) ── */
    async function run(type, btn) {
        ...
        try {
            ...
            if(type==='image'){
                const canvas=await _doCapture(area, false, originalTab);
                const link=document.createElement('a');
                link.download=`youtube_overview_${OV_PID}_${stamp}.png`;
            ...
            for(let i=0; i<TAB_ORDER.length; i++){
                ...
                const canvas=await _doCapture(area, false, key);
                canvases.push({label:`YouTube Overview — ${label}`, canvas});
            }

            /* Hitung total halaman */
            let totalPages=0;
            canvases.forEach(({canvas})=>{
                totalPages+=Math.max(1, Math.ceil((canvas.height*(uw/canvas.width))/uh));
            });
            ...
            canvases.forEach(({label,canvas})=>{
                ...
                while(srcY<canvas.height){
                    ...
                    _drawHeader(pdf, pW, pH, label, pageNum, totalPages);
                    const srcSlice=Math.min(sliceH, canvas.height-srcY), dstH=srcSlice*ratio;
                    const slice=document.createElement('canvas');
                    slice.width=canvas.width; slice.height=Math.ceil(srcSlice);
                    slice.getContext('2d').drawImage(canvas, 0,srcY, canvas.width,srcSlice, 0,0, canvas.width,srcSlice);
            ...
            pdf.save(`youtube_overview_${OV_PID}_${stamp}.pdf`);
    ...
    function _cardFilename(k){
        const map={
            'hashtag-list':'top-hashtags','hashtag-donut':'hashtag-distribusi',
            'donut-view':'distribusi-most-viewed','donut-like':'distribusi-most-liked','donut-comment':'distribusi-most-comments',
            'list-view':'top-videos-by-view','list-like':'top-videos-by-like','list-comment':'top-videos-by-comment',
        };
        return `youtube_${map[k]||k}_${OV_PID}_${_stamp()}`;
    }

    /* Tentukan tab key dari card key */
    function _tabKeyFromCard(cardKey) {
        if(cardKey.includes('hashtag')) return 'hashtag';
        if(cardKey.includes('view'))    return 'view';
        if(cardKey.includes('like'))    return 'like';
        if(cardKey.includes('comment')) return 'comment';
        return null;
    }
    ...
    async function runCard(areaId, cardKey, type, btn) {
        ...
        try {
            ...
            const tabKey=_tabKeyFromCard(cardKey);
            const canvas=await _doCapture(area, true, tabKey);
            ...
            if(type==='image'){
                ...
                if(fitsOne){
                    ...
                    const total=Math.max(1,Math.ceil((canvas.height*(uw/canvas.width))/uh));
                    _sliceCanvas(pdf,canvas,M,pW,pH,label,1,total);
                }
                pdf.save(fname+'.pdf');
