...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin:0; padding:0; }
  ...
  .eng-stat-card--total  { --bar-color: linear-gradient(90deg,#7c3aed,#5b21b6); }
  ...
  .eng-stat-icon-sm { width: 18px; height: 18px; flex-shrink: 0; }
  .eng-stat-value {
  ...
  /* ── PLATFORM LIST (Right sidebar style) ── */
  ...
  .eng-type-tabs {
    ...
    background: var(--bg-gray-100); border-radius: var(--radius-sm);
  ...
  .eng-type-tab:hover { background: var(--bg-white); }
  .eng-type-tab.active { background: var(--bg-white); color: var(--primary-green); box-shadow: 0 1px 4px rgba(0,0,0,.08); }
...
<div class="eng-page">
  ...
  <div class="eng-total-card">
    ...
    <div style="text-align:right;">
      <div style="font-size:12px;opacity:.8;margin-bottom:8px;">By Mention</div>
  ...
  <div class="eng-stat-grid" id="engStatGrid">
    @foreach([
      ...
      ['id'=>'statTwit',  'label'=>'Twitter',           'class'=>'eng-stat-card--twit',   'color'=>'#1d9bf0'],
  ...
  <div class="eng-grid-3-2 eng-mb20">
    ...
    {{-- Bar Chart: Total Interaction by Media Types --}}
    <div class="eng-card">
      <div class="eng-card-head">
        ...
        <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
          <button class="eng-csv-btn" onclick="ENGCsv.copyBar()" title="Copy CSV">
      ...
      <div class="eng-card-body">
        <div class="eng-ch-360" id="chBarWrap">
    ...
    {{-- Pie / Doughnut: Share of Interaction --}}
    <div class="eng-card">
      <div class="eng-card-head">
        <div class="eng-card-head-left">
          <span class="eng-head-icon">
      ...
      <div class="eng-card-body" style="display:flex;flex-direction:column;gap:16px;">
        ...
        {{-- Platform list like reference image --}}
        <div class="eng-plat-list" id="engPlatList">
          @foreach([
            ['News','#0284c7'],['Facebook','#1877f2'],['Twitter','#1d9bf0'],
            ['YouTube','#ff0000'],['Instagram','#e1306c'],['TikTok','#2dd4bf'],
          ] as [$name,$color])
  ...
  <div class="eng-card eng-mb20">
    ...
    <div class="eng-card-body">
      <div class="eng-ch-400" id="chTrendWrap">
  ...
  <div class="eng-grid-2">
    ...
    <div class="eng-card">
      <div class="eng-card-head">
        <div class="eng-card-head-left">
          <span class="eng-head-icon">
            <svg viewBox="0 0 24 24"><path d="M17 2H7a5 5 0 0 0-5 5v10a5 5 0 0 0 5 5h10a5 5 0 0 0 5-5V7a5 5 0 0 0-5-5z"/><circle cx="12" cy="12" r="3"/><circle cx="17.5" cy="6.5" r="1.5"/></svg>
          </span>
          <div>
...
const emptyHtml = msg => `<div class="eng-empty"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span class="eng-empty-text">${msg}</span></div>`;
...
const PLAT_META = {
  ...
  twitter:  { label:'Twitter',           color:'#1d9bf0', bg:'rgba(29,155,240,.1)',  icon:'<svg viewBox="0 0 24 24" fill="#1d9bf0" stroke="none" style="width:18px;height:18px;"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>', statId:'statTwit' },
...
══════════════════════════════════════════════════════ */
const ENGData = {
  platforms: [],   // [{key, label, count, color}]
  trend:     [],   // from trendMentions
  grandTotal: 0,
};
...
function renderBarChart() {
  ...
  const labels = ENGData.platforms.map(p => p.label);
  const values = ENGData.platforms.map(p => p.count);
  const colors = ENGData.platforms.map(p => p.color);
  ...
  chart.setOption({
    ...
    xAxis: {
      ...
      axisLabel: { fontFamily:"'Poppins',sans-serif", fontSize:11, fontWeight:'600', color:'#374151',
        rotate: labels.some(l=>l.length>8) ? 15 : 0, interval:0 }
    },
    yAxis: {
  ...
  chart.on('click', params => {
    const platform = ENGData.platforms[params.dataIndex];
    if (platform) highlightPlatform(platform.key);
  });
}
...
function renderPieChart() {
  ...
  const total = ENGData.grandTotal;
  if (total === 0) {
    document.getElementById('chPie').innerHTML = emptyHtml('Tidak ada data');
    return;
  }
...
/* ══════════════════════════════════════════════════════
   RENDER — PLATFORM LIST (right side)
══════════════════════════════════════════════════════ */
...
function renderTrendChart() {
  ...
  // Get dates from first platform with data
  ...
  if (legendEl) {
    legendEl.innerHTML = trendData.map(p =>
      `<div class="eng-legend-item"><span class="eng-legend-dot" style="background:${p.color};"></span>${p.label}</div>`
    ).join('');
  }
  ...
  const series = trendData.map(p => ({
    name: p.label,
    type: 'line',
    data: (p.data || []).map(d => d.count),
    smooth: .4,
    symbol: 'circle',
    symbolSize: dates.length <= 30 ? 5 : 0,
...
/* ══════════════════════════════════════════════════════
   RENDER — MASS vs SOCIAL CHARTS
══════════════════════════════════════════════════════ */
function renderMassSocialCharts() {
  const trendData = ENGData.trend;
  if (!trendData.length) {
    ['skMass','skSocial'].forEach(hideSk);
    return;
  }
  ...
  const massPlt   = trendData.filter(p => p.key === 'doc');
  const socialPlt = trendData.filter(p => p.key !== 'doc');

  renderGroupedBars('chMass', 'skMass', massPlt, dates, 'Mass Media');
  renderGroupedBars('chSocial', 'skSocial', socialPlt, dates, 'Social Media');
}

function renderGroupedBars(domId, skelId, platforms, dates, groupLabel) {
  hideSk(skelId);
  if (!platforms.length) {
    document.getElementById(domId).innerHTML = emptyHtml(`Tidak ada data ${groupLabel}`);
    return;
  }
  ...
  const totals = platforms.map(p => ({
    label: p.label,
    color: p.color,
    total: (p.data || []).reduce((s,d)=>s+d.count, 0),
  })).filter(p => p.total > 0);
  ...
  chart.setOption({
    ...
    xAxis: {
      type: 'category', data: totals.map(p=>p.label),
      axisLine:{show:false}, axisTick:{show:false},
      axisLabel:{ fontFamily:"'Poppins',sans-serif", fontSize:11, fontWeight:'600', color:'#64748b',
        interval:0, formatter: v => v.length>12 ? v.slice(0,11)+'…' : v }
    },
    yAxis: {
      type: 'value', axisLine:{show:false}, axisTick:{show:false},
      splitLine:{ lineStyle:{ color:'#f1f5f9', type:'dashed' } },
...
function highlightPlatform(key) {
  document.querySelectorAll('.eng-stat-card').forEach(c => c.style.opacity='0.5');
  const meta = PLAT_META[key];
  if (meta) {
    const el = document.getElementById(meta.statId);
    if (el) { el.style.opacity='1'; el.style.transform='translateY(-4px)'; el.style.boxShadow='var(--shadow-xl)'; }
  }
  setTimeout(() => {
    document.querySelectorAll('.eng-stat-card').forEach(c => { c.style.opacity='1'; c.style.transform=''; c.style.boxShadow=''; });
  }, 2000);
}
...
const ENGCsv = {
  ...
  copyBar() {
    ...
    ENGData.platforms.forEach(p => {
      lines.push(`${p.label};${p.count};${tot>0?(p.count/tot*100).toFixed(1):0}`);
    });
  ...
  copyTrend() {
    ...
    const rows    = dates.map((date, i) => {
      const vals = trendData.map(p => (p.data?.[i]?.count || 0));
