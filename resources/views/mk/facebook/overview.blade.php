...
<div id="pageExportArea">
    ...
    <div class="tme-tabs" id="ovTabsBar">
        ...
        <button class="tme-tab-btn" id="tab-share" onclick="OVTab.show('share')">
            <i class="ph ph-share-network"></i> Most Shared <span class="tme-tab-chip" id="chip-share">—</span>
        </button>
    ...
    @foreach(['like' => 'Most Liked', 'share' => 'Most Shared', 'comment' => 'Most Comments'] as $tp => $tpLabel)
    @php
...
<div class="do-panel" id="ovSntPanel">
    <div class="do-panel-header">
        ...
        <span class="do-panel-title" id="ovPanelTitle">Facebook Posts</span>
...
const PLAT     = 'Facebook';
const API_BASE = '/mk/api/facebook';
const OVCfg    = {
    pid    : OV_PID,
    sd     : OV_SD,
    ed     : OV_ED,
...
const Store = { like:[], share:[], comment:[], hashtag:[] };
const Pag   = { like:1,  share:1,  comment:1,  hashtag:1  };
let allPostsRaw = [];

window.addEventListener('resize', () => {
    ['__ec_donutHashtagChart','__ec_donutChart-like','__ec_donutChart-share','__ec_donutChart-comment']
...
const OVData = {
    ...
    _getColor(item) {
        const seed=item.sub_id||item.id||this._getName(item)||'fb';
        const pal=['#1877f2','#038047','#273B4A','#F59E0B','#06B6D4','#8b5cf6','#ec4899','#f97316'];
        ...
        ls.querySelectorAll('.tme-post').forEach(el => {
            el.addEventListener('click', () => {
                try {
                    ...
                    const lm={like:'Most Liked',share:'Most Shared',comment:'Most Comments'};
    ...
    _postHtml(item,gi,type) {
        ...
        const content=dec((item.content||item.name||'').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()).slice(0,200);
        const dt=(item.date_created||'').split('T')[0], url=item.url||item.link||'';
        const l=parseInt(item.likes||item.num_likes||item.freq||0),
              s=parseInt(item.shares||item.num_shares||0),
              c=parseInt(item.comments||item.num_comments||0), total=l+s+c;
        const sL={pos:'Positive',neg:'Negative',neu:'Neutral'}[sent];
        const enc=encodeURIComponent(JSON.stringify(item));
        const lC=type==='like'?' tme-metric--primary':'',
              sC=type==='share'?' tme-metric--cyan':'',
    ...
    _renderDonut(type, items) {
        ...
        const top5=items.slice(0,5), ml={like:'Likes',share:'Shares',comment:'Comments'}[type];
    ...
    /* ── Breakdown list di bawah donut (identik X/YT) ── */
    ...
    /* ── ECharts donut (identik X/YT: label ringkas, no legend header) ── */
    ...
    _openHashtagPanel(h) {
        ...
        const filtered=allPostsRaw.filter(p=>(p.content||p.name||'').toLowerCase().includes(tag));
...
const OVPanel = {
    ...
    _render(items,type) {
        ...
        const ml={like:'Likes',share:'Shares',comment:'Komentar'}[type]||'Likes';
        list.innerHTML=items.slice(0,100).map(item=>{
            const nm=OVData._getName(item), av=OVData._getAvatar(item), cl=OVData._getColor(item),
                  ...
                  tx=(item.content||item.name||'').replace(/<[^>]*>/g,'').trim(),
                  mv=OVData._metric(item,type), dt=(item.date_created||'').split('T')[0],
                  sn=OVData._normSent(item), sl={pos:'Pos',neg:'Neg',neu:'Neu'}[sn],
                  l=parseInt(item.likes||item.freq||0), s=parseInt(item.shares||0), c=parseInt(item.comments||0),
                  total=l+s+c, enc=encodeURIComponent(JSON.stringify(item));
...
const OVDetail = {
    open(item,type) {
        ...
        const color=OVCfg.colors[type]||OVCfg.primary, name=OVData._getName(item),
              ...
              d='/assets/images/user/dummy.jpg',
              aH=(av&&av.startsWith('http'))?`<img src="${esc(av)}" onerror="this.src='${d}'">`:`<img src="${d}">`;
        const raw=(item.content||item.name||'').replace(/<[^>]*>/g,'').trim(),
              ct=raw?dec(raw):'', url=item.url||item.link||'', dt=item.date_created||'';
        const l=parseInt(item.likes||item.num_likes||item.freq||0),
              s=parseInt(item.shares||item.num_shares||0),
              c=parseInt(item.comments||item.num_comments||0);
        ...
        body.innerHTML=`<div class="do-dp2-avatar-row">
            ...
            <div>
                ...
                <span class="do-dp2-plat-badge" style="background:#1877f218;color:#1877f2;">Facebook</span>
            </div>
        </div>
        ...
        ${url?`<a href="${esc(url)}" target="_blank" rel="noopener noreferrer" class="do-dp2-link"><i class="ph ph-arrow-square-out me-1"></i>Buka di Facebook</a>`:''}`;
...
/* ════════════════════════════════════════════════════════
   OVExport — FB Overview (identik X/YT: _snapshotForTab + skipSnapshot)
════════════════════════════════════════════════════════ */
const OVExport = (() => {
    'use strict';
    ...
    function _freeze(){
        if(document.getElementById('__fb_freeze')) return;
        const s=document.createElement('style'); s.id='__fb_freeze';
        s.textContent='*{animation:none!important;transition:none!important;animation-play-state:paused!important;}';
        document.head.appendChild(s);
    }
    function _unfreeze(){ document.getElementById('__fb_freeze')?.remove(); }
    ...
    function _snapshotForTab(tabKey) {
        ...
        (TAB_EC_MAP[tabKey]||[]).forEach(([key,containerId]) => {
            ...
            } catch(e){ console.warn('[OVExport] snapshot tab failed:',key,e); }
        });
    }
    ...
    function _preSnapshot() {
        ...
        Object.entries(EC_ID_MAP).forEach(([key,containerId]) => {
            ...
            } catch(e){ console.warn('[OVExport] snapshot failed:',key,e); }
        });
    }
    ...
    function _onClone(clonedDoc) {
        ...
        clonedDoc.querySelectorAll('.tme-post-av,.do-panel-avatar,.do-dp2-avatar-lg').forEach(wrapper => {
            ...
            if(!wrapper.querySelector('.__ini')){
                const txt=(wrapper.textContent||'').trim(), initial=txt?txt[0].toUpperCase():'F';
            ...
            if(!wrapper.style.background) wrapper.style.background='linear-gradient(135deg,#1877f2,#0d5fc2)';
        });
    ...
    function _drawHeader(pdf, pW, pH, label, page, total) {
        pdf.setFillColor(24,119,242); pdf.rect(0,0,pW,11,'F');
        pdf.setTextColor(255,255,255); pdf.setFontSize(9); pdf.setFont('helvetica','bold');
        pdf.text('SMADIMENT — '+(label||'Facebook Overview'), 10, 7.5);
        ...
        try {
            ...
            canvases.forEach(({label,canvas})=>{
                ...
                while(srcY<canvas.height){
                    ...
                    _drawHeader(pdf, pW, pH, `Facebook Overview — ${label}`, pageNum, totalPages);
            ...
            console.error('[OVExport.run]',err); _toast('Export gagal: '+err.message,'error');
        } finally {
    ...
    function _cardFilename(k){
        const map={
            ...
            'donut-like':'distribusi-most-liked','donut-share':'distribusi-most-shared','donut-comment':'distribusi-most-comments',
            'list-like':'top-posts-by-like','list-share':'top-posts-by-share','list-comment':'top-posts-by-comment',
        };
        const stamp=new Date().toISOString().slice(0,10).replace(/-/g,'');
        return `facebook_${map[k]||k}_${OV_PID}_${stamp}`;
    }
        ...
        try {
            ...
            if(type==='image'){
                ...
                if(fitsOne){ _drawHeader(pdf,pW,pH,label,1,1); _addCanvas(pdf,canvas,M,pW,pH); }
                    ...
                    const ratio=uw/canvas.width, sliceH=uh/ratio, total=Math.ceil((canvas.height*ratio)/uh);
                    let srcY=0, pg=1;
                    while(srcY<canvas.height){
                        if(pg>1) pdf.addPage();
