...
.fea-post-thumb { width:80px;height:80px;border-radius:var(--radius-sm);flex-shrink:0;overflow:hidden;border:1.5px solid var(--slate-200);background:var(--slate-800);position:relative;align-self:center;box-shadow:var(--shadow-sm); }
...
@section('page-title', 'Facebook Emotion Analysis')
        ...
        <div class="card h-100 text-white kpi-card-hover" style="background:{{ $kc['color'] }}; animation:fadeUp .38s ease-out {{ $delays[$ki] }} both;">
            <div class="card-body"><div class="d-flex align-items-center">
                <div class="flex-grow-1">
                    ...
                    <p class="mb-0 mt-2 text-white text-opacity-75 f-12">{!! str_replace('width="24" height="24"', 'width="12" height="12" style="vertical-align:text-bottom;margin-right:4px;"', $kc['icon']) !!}{{ $kc['sub'] }}</p>
                </div>
                <div class="flex-shrink-0 ms-3"><div class="kpi-icon-bg">{!! $kc['icon'] !!}</div></div>
            </div></div>
        </div>
    </div>
    @endforeach
...
const ECharts={};
function makeEChart(id){if(ECharts[id]){try{ECharts[id].dispose();}catch(e){}}const el=_$(id);if(!el)return null;el.style.display='block';const c=echarts.init(el,null,{renderer:'svg'});

ECharts[id]=c;window.addEventListener('resize',()=>{try{c.resize();}catch(e){}});return c;}
...
function detectEmotion(post){
    ...
    const content=(post.content||post.message||post.name||'').toLowerCase();
    const sent=(post.sentiment_str||'').toLowerCase();
    for(const[emo,kws]of Object.entries(EMO_KW)){if(kws.some(k=>content.includes(k)))return emo;}
...
function getColor(item){const seed=item.author_id||item.id||getName(item)||'fb';const palette=['#1877F2','#0d65d9','#10b981','#f59e0b','#8b5cf6','#06b6d4','#ec4899','#14b8a6','#f97316','#6366f1'];let h=0;for(let i=0;i<seed.length;i++)h=(h*31+seed.charCodeAt(i))&0xffffffff;return palette[Math.abs(h)%palette.length];}
...
const FEAData={
    ...
    async loadAll(){
        ...
        const url=`/mk/api/facebook/most-viewed-posts?project_id=${FEACfg.pid}&start_date=${FEACfg.sd}&end_date=${FEACfg.ed}&sub=postbylike&rows=${rows}`;
        try{
    ...
    _postHtml(post,gi){
        ...
        const content=dec((post.content||post.message||'').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()).slice(0,200);
        const dt=(post.date_created||'').split('T')[0],url=post.url||post.link||'';
        const l=getLikes(post),s=getShares(post),c=getComments(post),total=l+s+c;
...
const FBExport = (() => {
    ...
    // Identik dengan FMEExport._swapChartsIn()
    // Kedua chart (donut + radar) sudah pakai renderer:'svg' setelah fix
 async function _swapChartsIn(el) {
    const swaps = [];

    /* ── 1. ApexCharts → PNG via built-in dataURI() ── */
    const apexIds = ['barChart', 'trendsChart'];
    for (const id of apexIds) {
        ...
        try {
            ...
            console.warn('[FBExport] apex swap failed:', id, e);
        }
    }
    ...
    for (const [id, inst] of [['donutChart', window.__feaDonut], ['radarChart', window._feaRadarChart]]) {
        ...
        try {
            ...
            console.warn('[FBExport] echarts swap failed:', id, e);
        }
    }
    ...
    // ── _doCapture: identik FMEExport._doCapture ──
    async function _doCapture(el, isCard) {
    el.querySelectorAll('.kpi-card-hover,.fea-post,.card,[class*="col-"]')
      .forEach(e => { 
          e.style.opacity = '1'; 
    ...
    // ★ Swap chart SVG → PNG img di live DOM (async, tunggu sampai selesai)
    const swaps = await _swapChartsIn(el);

    // Tunggu img load ke DOM
    ...
    function _drawHeader(pdf, pW, pH, label, page, total) {
        ...
        pdf.text('SMADIMENT — ' + (label || 'Facebook Emotion Analysis'), 10, 7.5);
    ...
    async function run(type, btn) {
        ...
        try {
            ...
            if (type === 'image') {
                ...
                a.download = `facebook_emotion_${FEA_PID}_${stamp}.png`;
                ...
                if ((canvas.height * (uw / canvas.width)) <= uh) {
                    _drawHeader(pdf, pW, pH, 'Facebook Emotion Analysis', 1, 1);
                    _addCanvas(pdf, canvas, M, pW, pH);
                } else {
                    _paginate(pdf, canvas, M, pW, pH, () => 'Facebook Emotion Analysis');
                }
                pdf.save(`facebook_emotion_${FEA_PID}_${stamp}.pdf`);
                _toast('PDF berhasil diunduh!', 'success');
            }
        } catch(err) {
            console.error('[FBExport.run]', err);
    ...
    function _cardFilename(k) {
        ...
        return `facebook_${map[k] || k}_${FEA_PID}_${_stamp()}`;
    }
