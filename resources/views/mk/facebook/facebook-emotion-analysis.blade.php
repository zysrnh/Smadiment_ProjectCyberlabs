...
@section('title', 'Facebook Emotion Analysis - SMADIMENT')
...
<style>
  :root {
    ...
    --facebook-light:        rgba(24,119,242,.08);
    --facebook-border:       rgba(24,119,242,.2);
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .do-card--facebook::before { background:linear-gradient(90deg,#1877F2,#0d65d9); }
  ...
  .do-head-icon--facebook { background:var(--facebook-light); }
  .do-head-icon--facebook svg { stroke:#1877F2; }
  ...
  .do-badge--facebook { background:#e8f0fd; color:#1a56db; }
  ...
  .ms-tab-btn.active { background:linear-gradient(135deg,var(--facebook) 0%,var(--facebook-dark) 100%); color:#fff; box-shadow:0 4px 12px rgba(24,119,242,.25); }
  ...
  .fme-post:hover { background:#f0f7ff; }
  ...
  .fme-post-av { width:46px; height:46px; border-radius:50%; flex-shrink:0; background:linear-gradient(135deg,#1877F2,#0d65d9); color:#fff; font-weight:700; font-size:14px; display:flex; align-items:center; justify-content:center; border:2px solid var(--border-gray); overflow:hidden; }
  ...
  .fme-post-metric--facebook { background:var(--facebook-light); border-color:var(--facebook-border); color:#1877F2; }
  .fme-post-metric--facebook svg { stroke:#1877F2; }
  ...
  .fme-view-link { display:inline-flex; align-items:center; gap:4px; font-size:10px; font-weight:700; color:#1877F2; text-decoration:none; padding:3px 9px; border-radius:20px; background:var(--facebook-light); border:1px solid var(--facebook-border); transition:var(--transition); margin-left:auto; }
  ...
  /* THUMBNAIL — Facebook square-ish */
  .fme-post-thumb { width:80px; height:80px; border-radius:10px; flex-shrink:0; overflow:hidden; border:2px solid var(--border-gray); background:#1a202c; position:relative; align-self:center; box-shadow:0 4px 12px rgba(0,0,0,.08); }
  ...
  .fme-post-thumb-placeholder { width:100%; height:100%; display:flex; align-items:center; justify-content:center; font-size:22px; background:linear-gradient(135deg,#1877F2,#0d65d9); }
  .fme-post-thumb-play { position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,.28); opacity:0; transition:opacity .2s; border-radius:8px; }
  .fme-post-thumb-play svg { width:22px; height:22px; fill:#fff; filter:drop-shadow(0 2px 6px rgba(0,0,0,.7)); }
  ...
  .fme-pag-btn:hover:not(:disabled):not(.is-active) { border-color:var(--facebook); color:var(--facebook); background:var(--facebook-light); }
  .fme-pag-btn.is-active { background:var(--facebook); border-color:var(--facebook); color:#fff; }
  ...
  .modal-header { padding:18px 24px; border-bottom:1px solid var(--border-gray); display:flex; justify-content:space-between; align-items:center; flex-shrink:0; background:linear-gradient(135deg,var(--facebook) 0%,var(--facebook-dark) 100%); }
  ...
  .modal-stat-value.v-likes { color:#1877F2; }
  .modal-stat-value.v-shares { color:#10b981; }
  ...
  .modal-btn.primary   { background:linear-gradient(135deg,var(--facebook),var(--facebook-dark)); color:#fff; box-shadow:0 4px 14px rgba(24,119,242,.25); }
  .modal-btn.primary:hover { transform:translateY(-2px); box-shadow:0 6px 20px rgba(24,119,242,.35); }
...
<div class="fme-page">
  ...
  <div class="page-header">
    <div class="page-header-left">
      <h1>
        <svg viewBox="0 0 24 24" style="width:26px;height:26px;display:inline-block;vertical-align:middle;margin-right:10px;margin-top:-3px;fill:#1877F2;stroke:none;">
          <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
        </svg>
        Facebook Emotion Analysis
      </h1>
      <p>Analisis pola emosi pada postingan Facebook berdasarkan Plutchik Wheel — joy, trust, fear, surprise, sadness, disgust, anger, anticipation</p>
    </div>
  ...
  <div class="filter-card">
    <form id="filterForm" method="GET" action="{{ route('mk.facebook.emotion-analysis') }}">
  ...
  <div class="ms-grid-2 ms-mb20">
    <div class="do-card do-card--facebook">
      <div class="do-card-head">
        <div class="do-card-head-left">
          <div class="do-head-icon do-head-icon--facebook">
        ...
        <span class="do-badge do-badge--facebook" id="barBadge">—</span>
      </div>
  ...
  {{-- TRENDS --}}
  <div class="do-card do-card--facebook ms-mb20">
    <div class="do-card-head">
      <div class="do-card-head-left">
        <div class="do-head-icon do-head-icon--facebook">
      ...
      <span class="do-badge do-badge--facebook">Trends</span>
    </div>
    <div class="do-card-body">
      <div class="ms-ch-340">
  ...
  <div class="do-card" id="tableCard">
    ...
    <div id="listSkel">
      ...
      <div style="display:flex;gap:14px;padding:16px 20px;border-bottom:1px solid #f1f5f9;align-items:flex-start;">
        ...
        <div class="loading-skeleton" style="width:80px;height:80px;border-radius:10px;flex-shrink:0;"></div>
      </div>
      @endfor
    </div>
...
function detectEmotion(post) {
  ...
  const content=(post.content||post.caption||post.name||'').toLowerCase();
...
/* FIELD HELPERS — Facebook specific */
...
function getThumbnail(item) {
  /* 1. Field langsung dari API */
  const direct=(item.image_url||item.thumbnail||item.thumb_url||item.thumb||item.image||'').trim();
  if(direct && direct.startsWith('http')) return direct;
  /* 2. Coba dari profile pic (Facebook post biasanya tidak ada thumbnail video) */
...
function getInitials(name) {
  ...
  return (w[0]?.[0]||'F').toUpperCase();
}
function getAvatarColor(item) {
  const seed=item.author_id||item.id||getName(item)||'fb';
  const palette=['#1877F2','#0d65d9','#10b981','#f59e0b','#8b5cf6','#06b6d4','#ec4899','#14b8a6','#f97316','#6366f1'];
...
const EmoApp = {
  ...
  async loadData() {
    ...
    /* Gunakan endpoint Facebook most-viewed-posts */
    const url=`/mk/api/facebook/most-viewed-posts?project_id=${EmoCfg.pid}&start_date=${EmoCfg.sd}&end_date=${EmoCfg.ed}`;
  ...
  postHTML(post, rank) {
    ...
    const thumbHtml = (thumb&&thumb.startsWith('http'))
      ? `<div class="fme-post-thumb">
           <img src="${esc(thumb)}" alt="thumb" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\'fme-post-thumb-placeholder\\'>f</div>'">
           <div class="fme-post-thumb-play"><svg viewBox="0 0 24 24"><polygon points="5 3 19 12 5 21 5 3"/></svg></div>
         </div>`
      : `<div class="fme-post-thumb"><div class="fme-post-thumb-placeholder" style="font-size:20px;font-weight:900;color:#fff;font-family:Georgia,serif;">f</div></div>`;

    const handle  = post.author?.scr_name||post.author_scr_name||'';
    const content = decodeStr((post.content||post.name||'').replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim()).slice(0,200);
    ...
    const totalEng = likes + shares + cmts;
    ...
    return `<div class="fme-post">
      ...
      <div class="fme-post-body">
        ...
        ${content?`<div class="fme-post-text">${esc(content)}</div>`:'<div class="fme-post-text" style="color:var(--text-muted);font-style:italic;">(Tidak ada konten)</div>'}
        <div class="fme-post-stats">
          ...
          <span class="fme-post-metric fme-post-metric--facebook"><svg viewBox="0 0 24 24"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3H14z"/><path d="M7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"/></svg>${numFmt(likes)}</span>
          <span class="fme-post-metric fme-post-metric--green"><svg viewBox="0 0 24 24"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>${numFmt(shares)}</span>
          ...
          ${url?`<a href="${esc(url)}" target="_blank" rel="noopener" class="fme-view-link" onclick="event.stopPropagation()"><svg viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>Facebook</a>`:''}
        </div>
      </div>
      ${thumbHtml}
    </div>`;
  ...
  openModal(post) {
    ...
    const safeIni = ini.replace(/['"\\]/g,'');
    const handle  = post.author?.scr_name||post.author_scr_name||'';
    const content = decodeStr((post.content||post.name||'').replace(/<[^>]*>/g,'').trim());
    const url     = post.url||post.link||'';
    ...
    /* Thumbnail untuk Facebook (tidak ada embed video) */
    const thumb = getThumbnail(post);
    const mediaHtml = thumb ? `
      <div>
        <div class="msdp-section-heading">
          <div class="msdp-section-heading-icon" style="background:rgba(24,119,242,.08);">
            <svg viewBox="0 0 24 24" style="stroke:#1877F2;stroke-width:2;fill:none;stroke-linecap:round;"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
          </div>
          <span class="msdp-section-title">Gambar Post</span>
        ...
        <div style="border-radius:14px;overflow:hidden;border:1px solid #e8edf2;box-shadow:0 4px 16px rgba(0,0,0,.07);">
          <img src="${esc(thumb)}" alt="post image" style="width:100%;display:block;max-height:320px;object-fit:cover;" onerror="this.parentElement.style.display='none'">
        </div>
      </div>` : '';
    ...
    document.getElementById('modalTitle').textContent=name||'Detail Post';
    document.getElementById('modalBody').innerHTML=`
      <div style="display:flex;flex-direction:column;gap:16px;">
        ...
        <div class="modal-stats-grid">
          ...
          <div class="modal-stat-box"><div class="modal-stat-value v-shares">${numFmt(shares)}</div><div class="modal-stat-label">Shares</div></div>
        ...
        ${content?`<div>
          <div class="msdp-section-heading">
            ...
            <span class="msdp-section-title">Konten Post</span>
            <div class="msdp-section-line"></div>
          </div>
        ...
        <div>
          <div class="msdp-section-heading">
            <div class="msdp-section-heading-icon" style="background:rgba(24,119,242,.08);">
              <svg viewBox="0 0 24 24" style="stroke:#1877F2;stroke-width:2;fill:none;stroke-linecap:round;"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
            </div>
        ...
        <div class="modal-actions">
          ${url?`<a href="${esc(url)}" target="_blank" rel="noopener" class="modal-btn primary"><svg viewBox="0 0 24 24" style="width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>Buka di Facebook</a>`:''}
  ...
  exportCSV() {
    ...
    const header='index;author;handle;emosi;sentimen;likes;shares;comments;tanggal;url;konten';
    const rows=filteredPosts.map((p,i)=>{
      ...
      return [i+1,`"${name.replace(/"/g,'""')}"`,`"${handle.replace(/"/g,'""')}"`,p.emotion||'',sent,
        getLikes(p), getShares(p), getComments(p),
        (p.date_created||'').split('T')[0], p.url||p.link||'',
        `"${(p.content||p.name||'').replace(/<[^>]*>/g,'').trim().replace(/"/g,'""').replace(/\n/g,' ').slice(0,300)}"`
      ].join(';');
    });
    const blob=new Blob(['\uFEFF'+[header,...rows].join('\r\n')],{type:'text/csv;charset=utf-8;'});
    const a=document.createElement('a'); a.href=URL.createObjectURL(blob);
    a.download=`Facebook_EmotionAnalysis_${EmoCfg.sd}_${EmoCfg.ed}.csv`; a.click();
  },
  ...
  renderBarChart() {
    ...
    chart.setOption({
      ...
      tooltip:{ ...EC_TT, trigger:'axis', axisPointer:{type:'shadow',shadowStyle:{color:'rgba(24,119,242,.04)'}},
  ...
  renderRadarChart() {
    ...
    chart.setOption({
      ...
      series:[{ type:'radar', data:[{ value:EMOTIONS.map(e=>counts[e]||0), name:'Emotion',
        areaStyle:{color:{type:'linear',x:0,y:0,x2:1,y2:1,colorStops:[{offset:0,color:'rgba(24,119,242,0.18)'},{offset:1,color:'rgba(13,101,217,0.08)'}]}},
        lineStyle:{color:'#1877F2',width:2.5}, symbol:'circle', symbolSize:6, itemStyle:{color:EMOTIONS.map(e=>EMO_COLORS[e]),borderColor:'#fff',borderWidth:2} }] }]
    });
  },
