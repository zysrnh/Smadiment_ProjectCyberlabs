...
@section('title', 'Facebook Word Cloud - SMADIMENT')
...
@section('page-title', 'Facebook Word Cloud')
...
<div class="page-export-bar" data-html2canvas-ignore="true">
    ...
    <div class="page-export-bar-right">
        <button type="button" class="page-export-btn page-export-btn-pdf" id="pageExportPdfBtn" onclick="FBWCExport.run('pdf', this)" title="Export halaman sebagai PDF">
            <i class="ph ph-file-pdf exp-icon"></i><span class="exp-spinner"></span>
        </button>
        <button type="button" class="page-export-btn page-export-btn-img" id="pageExportImgBtn" onclick="FBWCExport.run('image', this)" title="Export halaman sebagai PNG">
                ...
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Klik kata untuk cari di Facebook</small>
                        </div>
                    </div>
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div data-html2canvas-ignore="true" class="d-flex gap-1">
                            <button class="card-exp-btn card-exp-btn-pdf" onclick="FBWCExport.runCard('card-export-wc','wordcloud','pdf',this)" title="Export Word Cloud PDF">
                                <i class="ph ph-file-pdf exp-icon"></i><span class="exp-spinner"></span>
                            </button>
                            <button class="card-exp-btn card-exp-btn-img" onclick="FBWCExport.runCard('card-export-wc','wordcloud','image',this)" title="Export Word Cloud PNG">
                                <i class="ph ph-image exp-icon"></i><span class="exp-spinner"></span>
                            </button>
                        </div>
                    </div>
        ...
        <div class="card mb-3" style="animation:fadeUp .38s ease-out .22s both;">
            <div id="card-export-topics">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <div data-html2canvas-ignore="true" class="d-flex gap-1">
                        <button class="card-exp-btn card-exp-btn-pdf" onclick="FBWCExport.runCard('card-export-topics','topics','pdf',this)" title="Export Topics PDF">
                            <i class="ph ph-file-pdf exp-icon"></i><span class="exp-spinner"></span>
                        </button>
                        <button class="card-exp-btn card-exp-btn-img" onclick="FBWCExport.runCard('card-export-topics','topics','image',this)" title="Export Topics PNG">
                            <i class="ph ph-image exp-icon"></i><span class="exp-spinner"></span>
                        </button>
                    </div>
                </div>
...
const numF = n => parseInt(n||0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.');
...
/* ══════════════════════════════════════════════════════════════
   SENTIMENT DETECTION v3
   ─────────────────────────────────────────────────────────────
   Perubahan dari versi sebelumnya:
   1. Pakai Set → lookup O(1) per token
   2. Tokenisasi word-boundary (split /[\s_\-#@]+/) → tidak ada
      false-positive dari substring (mis. "baik" di "pembaikhati")
   3. Tie (termasuk 0-0) → "neutral", BUKAN default positive
   4. Keyword negatif lebih lengkap & spesifik
   ══════════════════════════════════════════════════════════════ */
...
/**
 * Tokenisasi: hilangkan awalan # @ lalu split berdasarkan
 * spasi, underscore, dash, dan karakter non-alfanumerik.
 * Setiap token dicocokkan secara eksak dengan Set (bukan substring).
 *
 * @param {string} name – nama topik/hashtag
 * @returns {'positive'|'negative'|'neutral'}
 */
function getSent(name) {
    const clean  = name.toLowerCase().replace(/^[#@]+/, '');
    /* Tokenisasi: split on non-alphanumeric boundaries */
    ...
    /* Juga cek kata gabung (tanpa delimiter) jika token tunggal panjang */
    ...
    return 'neutral'; /* tie atau tidak ada match → neutral */
}

/* ══ Load Data ══ */
async function loadData() {
    try {
        const r = await fetch(`/mk/api/facebook/trending-topics?project_id=${CFG.pid}&start_date=${CFG.sd}&end_date=${CFG.ed}`);
...
function renderWC() {
    ...
    requestAnimationFrame(() => {
        ...
        const colorsBySent = {
            ...
            neutral:  ['#b45309','#d97706','#f59e0b','#fbbf24','#92400e','#fde68a'],
            all:      ['#16a34a','#2563eb','#f59e0b','#ef4444','#9333ea','#14b8a6','#1877f2','#e1306c'],
        };
        ...
        wcChart.on('click', p => {
            ...
            window.open(`https://www.facebook.com/search/posts?q=${q}`, '_blank', 'noopener,noreferrer');
        });

        if (window._fbwcResizeObserver) window._fbwcResizeObserver.disconnect();
        if (typeof ResizeObserver !== 'undefined') {
            window._fbwcResizeObserver = new ResizeObserver(() => {
                if (wcChart) {
            ...
            window._fbwcResizeObserver.observe(ch.parentElement);
        }
    });
}
...
function renderList() {
    ...
    items.forEach((h, i) => {
        ...
        const sentColor = h.sent === 'positive' ? '#16a34a'
                        : h.sent === 'negative' ? '#dc2626'
                        : '#b45309'; /* neutral = amber */
        ...
        el.onclick = () => { const q = encodeURIComponent(h.name.replace(/^#/,'')); window.open(`https://www.facebook.com/search/posts?q=${q}`,'_blank','noopener,noreferrer'); };
        list.appendChild(el);
    });
    if (pg) {
...
const FBWCExport = (() => {
    ...
    function _paginatePdf(pdf, canvas, title) {
        ...
        while (srcY < canvas.height) {
            if (pg > 0) { pdf.addPage(); _pdfHeader(pdf, title); }
    ...
    async function run(type, btn) {
        ...
        try {
            ...
            if (type === 'image') {
                ...
                a.download = `facebook_wordcloud_${PID}_${stamp}.png`;
                a.href = canvas.toDataURL('image/png'); a.click();
                _toast('Gambar berhasil diunduh!', 'success');
            } else {
                const { jsPDF } = window.jspdf;
                const pdf = new jsPDF({ orientation:'portrait', unit:'mm', format:'a4' });
                _pdfHeader(pdf, 'Facebook Word Cloud');
                _paginatePdf(pdf, canvas, 'Facebook Word Cloud');
                pdf.save(`facebook_wordcloud_${PID}_${stamp}.pdf`);
                _toast('PDF berhasil diunduh!', 'success');
            }
        } catch(err) {
            console.error('[FBWCExport]', err);
    ...
    async function runCard(areaId, cardKey, type, btn) {
        ...
        try {
            ...
            const titles = { wordcloud:'Facebook Word Cloud', topics:'Facebook Top Topics' };
            const fname  = `facebook_${labels[cardKey]||cardKey}_${PID}_${_stamp()}`;
            const title  = titles[cardKey] || 'Facebook Word Cloud';
            if (type === 'image') {
                const a = document.createElement('a');
                ...
                _pdfHeader(pdf, title);
                _paginatePdf(pdf, canvas, title);
                pdf.save(fname + '.pdf');
                _toast('PDF berhasil diunduh!', 'success');
            }
        } catch(err) {
            console.error('[FBWCExport.runCard]', err);
            _toast('Export gagal: ' + err.message, 'error');
