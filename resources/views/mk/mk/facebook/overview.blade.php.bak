...
<div class="dashboard-container">
  ...
  <div class="page-header">
    <h1>
      <svg viewBox="0 0 24 24" style="width:28px;height:28px;fill:#1877F2;vertical-align:middle;margin-right:8px;">
        <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
      </svg>
      Facebook Overview Dashboard
    </h1>
    <p>Monitor and analyze your Facebook social media performance metrics</p>
  </div>
  ...
  <div class="alert alert-warning">
    ...
    <span>No project selected. Please select a project from the sidebar to view Facebook Overview data.</span>
  </div>
  @else

  <!-- Date Filter Card -->
  <div class="filter-card">
    <form id="filterForm" method="GET" action="{{ route('mk.facebook.overview') }}">
...
<script>
  ...
  if (projectId && startDate && endDate) {
    ...
    async function loadTotalUsers() {
      ...
      try {
        const response = await fetch(`/mk/api/facebook/total-users?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
    ...
    async function loadTotalAuthors() {
      ...
      try {
        const response = await fetch(`/mk/api/facebook/total-authors?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
        const result = await response.json();
    ...
    async function loadVolumeTotal() {
      ...
      try {
        const response = await fetch(`/mk/api/facebook/volume-total?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
        const result = await response.json();
    ...
    async function loadSentimentTotal() {
      ...
      try {
        const response = await fetch(`/mk/api/facebook/sentiment-total?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
        const result = await response.json();
    ...
    async function loadMostActiveUsers() {
      ...
      try {
        const response = await fetch(`/mk/api/facebook/most-active-users?project_id=${projectId}&start_date=${startDate}&end_date=${endDate}`);
        const result = await response.json();
    ...
    function displayUsersTable(count) {
      ...
      html += '<th>NO.</th><th>AVATAR</th><th>NAME</th><th>POSTS</th><th>LIKES</th><th>SHARES</th><th>COMMENTS</th></tr></thead><tbody>';
      ...
      users.forEach((item, index) => {
        ...
        html += `<tr>
          ...
          <td>
            <a href="https://facebook.com/${username}" target="_blank" class="account-name-link">${username}</a>
          </td>
    ...
    function filterUsers() {
      ...
      if (filteredUsers.length === 0) {
        html += '<tr><td colspan="7" style="text-align: center; padding: 40px; color: var(--text-secondary);">No users found matching "' + searchTerm + '"</td></tr>';
      } else {
  ...
  function exportUsers() {
    ...
    let csvContent = "No.,Name,Posts,Likes,Shares,Comments\n";
    
    allUsers.forEach((item, index) => {
      const username = (item.username || item.name || 'Unknown').replace(/,/g, ' ');
      const posts = item.posts || item.y || 0;
      const likes = item.likes || 0;
      const shares = item.shares || 0;
      const comments = item.comments || 0;
      
      csvContent += `${index + 1},"${username}",${posts},${likes},${shares},${comments}\n`;
    });
    ...
    link.setAttribute('download', `facebook_active_users_${startDate}_${endDate}.csv`);
  ...
  function printTable() {
    ...
    printWindow.document.write(`
      ...
      <html>
      <head>
        <title>Most Active Users - Facebook Overview</title>
        <style>
      ...
      <body>
        <h1>Most Active Users - Facebook</h1>
