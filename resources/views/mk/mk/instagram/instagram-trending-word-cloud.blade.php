...
@section('title', 'Instagram Word Cloud - SMADIMENT')
...
@section('page-title', 'Instagram Word Cloud')
...
<div class="page-export-bar" data-html2canvas-ignore="true">
    ...
    <div class="page-export-bar-right">
        <button type="button" class="page-export-btn page-export-btn-pdf" id="pageExportPdfBtn" onclick="IGWCExport.run('pdf', this)" title="Export halaman sebagai PDF">
            <i class="ph ph-file-pdf exp-icon"></i><span class="exp-spinner"></span>
        </button>
        <button type="button" class="page-export-btn page-export-btn-img" id="pageExportImgBtn" onclick="IGWCExport.run('image', this)" title="Export halaman sebagai PNG">
                ...
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Klik kata untuk cari di Instagram</small>
                        </div>
                    </div>
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div data-html2canvas-ignore="true" class="d-flex gap-1">
                            ...
                            <button class="card-exp-btn card-exp-btn-img" onclick="IGWCExport.runCard('card-export-wc','wordcloud','image',this)" title="Export Word Cloud PNG">
                                <i class="ph ph-image exp-icon"></i><span class="exp-spinner"></span>
                            </button>
                        </div>
                    </div>
        ...
        <div class="card mb-3" style="animation:fadeUp .38s ease-out .22s both;">
            <div id="card-export-topics">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <div data-html2canvas-ignore="true" class="d-flex gap-1">
                        <button class="card-exp-btn card-exp-btn-pdf" onclick="IGWCExport.runCard('card-export-topics','topics','pdf',this)" title="Export Topics PDF">
                            <i class="ph ph-file-pdf exp-icon"></i><span class="exp-spinner"></span>
                        </button>
                        <button class="card-exp-btn card-exp-btn-img" onclick="IGWCExport.runCard('card-export-topics','topics','image',this)" title="Export Topics PNG">
                            <i class="ph ph-image exp-icon"></i><span class="exp-spinner"></span>
                        </button>
                    </div>
                </div>
...
const NEG_KW = [
    ...
    'homeless','hungry','poverty','poor','underpaid','underprivileged',
    /* ID */
    ...
    'gagalkan','kegagalan','kekacauan','kehancuran','kekerasan','kebencian',
    'permusuhan','konflik','sengketa','saling','serangan','teror','ancam',
];
...
const POS_KW = [
    ...
    'resmi','diluncurkan','diresmikan','terpilih','meraih','mendapatkan',
    'diraih','berhasil','sukses','lolos','lulus','diterima','dipuji',
];
...
function getSent(name) {
    ...
    return 'positive'; /* default positive jika tie */
}

/* ══ Load Data ══ */
async function loadData() {
    try {
        const r = await fetch(`/mk/api/instagram/trending-topics?project_id=${CFG.pid}&start_date=${CFG.sd}&end_date=${CFG.ed}`);
...
function renderWC() {
    ...
    requestAnimationFrame(() => {
        ...
        wcChart.on('click', p => {
            ...
            window.open(`https://www.instagram.com/explore/tags/${q}`, '_blank', 'noopener,noreferrer');
        });

        if (window._igwcResizeObserver) window._igwcResizeObserver.disconnect();
        if (typeof ResizeObserver !== 'undefined') {
            window._igwcResizeObserver = new ResizeObserver(() => {
                if (wcChart) {
            ...
            window._igwcResizeObserver.observe(ch.parentElement);
        }
    });
}
...
function renderList() {
    ...
    items.forEach((h, i) => {
        ...
        el.onclick = () => { const q = encodeURIComponent(h.name.replace(/^#/,'')); window.open(`https://www.instagram.com/explore/tags/${q}`,'_blank','noopener,noreferrer'); };
        list.appendChild(el);
    });
    if (pg) {
...
const IGWCExport = (() => {
    ...
    async function runCard(areaId, cardKey, type, btn) {
        ...
        try {
            ...
            console.error('[IGWCExport.runCard]', err);
            _toast('Export gagal: ' + err.message, 'error');
